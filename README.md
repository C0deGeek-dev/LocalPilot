...
| `localpilot mesh …` | Take part in a Claude Code / Codex pair-programming session through its shared file mailbox; see [Pairing with Claude Code and Codex](docs/configuration.md#pairing-with-claude-code-and-codex-localpilot-mesh) |
...
When embeddings are configured, a session-bearing command owns their lifecycle:
LocalPilot may start LocalBox, while both LocalPilot and standalone LocalMind
commands share exact endpoint/PID leases. A server you started yourself remains
user-managed and is never stopped by this lifecycle. If another leased command
outlives the session, a detached LocalPilot reaper waits for it before stopping
only the server LocalPilot registered.
...
the interactive terminal. Windows release and nightly binaries also enable the
native credential-manager backend. Enable it in a source build with:
