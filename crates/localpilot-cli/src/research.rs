...
use localpilot_research::{
    ...
    AdmissionTrail, AuditEntry, Bounds, ClaimStatus, CoverageVerdict, EgressOutcome, EgressRequest,
    Evidence, Finding, Gathered, HeuristicSynthesizer, Provenance, RenderBounds, RenderGate,
    RenderRequest, RenderSignal, Renderer, ResearchError, ResearchReport, RunControl, Source,
    SourceAccount, SourceError, SourceSet, Synthesizer, WebAccess,
};
...
pub async fn run_research_command_with_config(
    ...
    let skill_style = localpilot_skills::PlainSkillCatalogStyle;
    if let Err(error) =
        crate::skill_discovery::run_skill_research(root, topic, false, web, &skill_style, out).await
    {
...
fn write_research_disclosure(
    ...
    if web_config.enabled {
        if web_config.allowlist.is_empty() {
            writeln!(
                out,
                "  reach: you will be asked before each new host — allow once, deny, \
                 or allow every request for this session"
            )?;
            writeln!(
                out,
                "  pre-approve hosts with [research.web].allowlist; with no terminal                  to ask, nothing outside it is fetched"
            )?;
            ...
            writeln!(
                out,
                "  non-allowlisted hosts are asked about, then skipped if refused"
            )?;
        }
...
///
/// The source is **inert** (every host resolves to [`EgressOutcome::Off`])
...
async fn build_web_source(
    ...
    // A run launched with every prompt pre-approved has already answered this
    // one. The grant is session-wide from the start rather than asked for on the
    // first request — the flag exists precisely so nobody is interrupted.
    //
    // It does not lift `disallowlist`, and it cannot revive a run whose config
    // has web off: both of those are refusals written down in advance, and this
    // answers questions rather than overriding answers.
    if crate::bypass::engaged() {
        access.grant_session_wide();
    }
    // Otherwise, attach whoever will answer. With no terminal on both ends there
    // is nobody to ask, and `WebAccess` already defaults to refusing in that
    // case — so a piped or scheduled run reaches allowlisted hosts and nothing
    // else.
    else if crate::egress_consent::someone_to_ask() {
        access = access.with_approval(localpilot_research::shared_approval(
            crate::egress_consent::TerminalApproval::new(
...
impl WebSource {
    ...
    async fn recover_frames(
        ...
        for src in lead.iframe_srcs.iter().take(FRAME_RECOVERY_LIMIT) {
            ...
            match self.access.decide(&EgressRequest {
                host: &frame_host,
                url: &frame_url,
                purpose: ctx.query,
            }) {
...
fn decide_destination(
    ...
    // A redirect is a *new* destination, so it is asked about like any other.
    // Following a hop the operator never saw, on the strength of an approval they
    // gave for a different host, is exactly the reach the prompt exists to stop.
    match access.decide(&EgressRequest {
        host,
        url: parsed.as_str(),
        purpose,
    }) {
        EgressOutcome::Fetch => DestinationVerdict::Allowed,
        EgressOutcome::Skip | EgressOutcome::Off => DestinationVerdict::Blocked,
    }
}
...
impl RenderGate for WebAccessGate {
    fn allow(&self, url: &str) -> bool {
        ...
        let (decision, allowed) =
            match decide_destination(&self.access, &parsed, None, "rendering a page") {
    ...
    async fn gather(&self, question: &str, limit: usize) -> Result<Gathered, SourceError> {
        ...
        for url in urls.into_iter().take(limit) {
            ...
            match self.access.decide(&EgressRequest {
                ...
                EgressOutcome::Fetch => match self.fetch(&url, &host, question, &query).await? {
                    FetchOutcome::Fetched(page) => {
                        let FetchedPage {
                            evidence: found,
                            lead,
                ...
                // Web access is off entirely, so no later source can succeed
                // either. A *refused* host only skips itself.
                EgressOutcome::Off => return Ok(Gathered { evidence, account }),
            }
        }
        Ok(Gathered { evidence, account })
    }
...
mod tests {
    ...
    // --- egress consent, end to end through the real gather loop -------------

    fn scripted(
        answers: Vec<localpilot_research::ConsentAnswer>,
    ) -> (
        localpilot_research::SharedApproval,
        Arc<std::sync::atomic::AtomicUsize>,
    ) {
        let asked = Arc::new(std::sync::atomic::AtomicUsize::new(0));
        let approval = localpilot_research::shared_approval(ScriptedApproval {
            answers,
    ...
    #[tokio::test]
    async fn approving_a_host_fetches_it_and_records_the_fetch() {
        let server = ok_server("body").await;
        let url = format!("{}/page", server.uri());
        let dir = tempfile::tempdir().unwrap();
        let audit = dir.path().join("audit.log");
        // Nothing is allowlisted, so the only way this host is reached is a
        // person saying yes.
        let (approval, asked) = scripted(vec![localpilot_research::ConsentAnswer::AllowOnce]);
        let (source, _fake) = web_source_with(
            &url,
        ...
        assert!(
            ...
            "an approved host is actually fetched"
        );
        assert_eq!(gathered.account.policy_skipped, 0);
    ...
    #[tokio::test]
    async fn refusing_a_host_sends_nothing_and_still_records_the_refusal() {
        let server = ok_server("body").await;
        let url = format!("{}/page", server.uri());
        let dir = tempfile::tempdir().unwrap();
        let audit = dir.path().join("audit.log");
        let (approval, asked) = scripted(vec![localpilot_research::ConsentAnswer::Deny]);
        let (source, _fake) = web_source_with(
            &url,
            vec!["docs.rs".to_string()],
            true,
        ...
        assert!(
            server.received_requests().await.unwrap().is_empty(),
            "a refused host is never contacted"
        );
        assert_eq!(
            gathered.account.policy_skipped, 1,
            "a refusal is a countable skip, not a silent nothing"
        );
        // The half that is easy to lose: a refusal nobody can see afterwards is
        // not a record of anything.
        let log = std::fs::read_to_string(&audit).unwrap();
        assert!(
            log.contains("decision=skipped"),
            "the refusal is audited: {log}"
        );
    }

    #[tokio::test]
    async fn a_session_wide_grant_is_asked_for_once_and_then_not_again() {
        let server = ok_server("body").await;
        let url = format!("{}/page", server.uri());
        let dir = tempfile::tempdir().unwrap();
        let audit = dir.path().join("audit.log");
        let (approval, asked) =
            scripted(vec![localpilot_research::ConsentAnswer::AllowSessionWide]);
        let (source, _fake) = web_source_with(
            &url,
            vec!["docs.rs".to_string()],
            true,
        ...
        // Two gathers on one source: the grant has to survive the first.
        let _ = source.gather("q", 3).await.unwrap();
        let _ = source.gather("q2", 3).await.unwrap();

        assert_eq!(
            asked.load(std::sync::atomic::Ordering::Relaxed),
            1,
            "allow-all means the operator is asked once, not once per round"
        );
        assert!(!server.received_requests().await.unwrap().is_empty());
    }

    #[tokio::test]
    async fn with_nobody_to_ask_a_run_reaches_nothing_it_was_not_already_given() {
        // The default approval, which is what every piped, scheduled and CI run
        // gets. This is the behaviour change the plan asks someone to accept.
        let server = ok_server("body").await;
        let url = format!("{}/page", server.uri());
        let dir = tempfile::tempdir().unwrap();
        let audit = dir.path().join("audit.log");
        ...
        assert!(server.received_requests().await.unwrap().is_empty());
        assert_eq!(gathered.account.policy_skipped, 1);
        let log = std::fs::read_to_string(&audit).unwrap();
        assert!(log.contains("decision=skipped"));
    }

    /// Every place in the workspace that can build an outbound HTTP client, and
    /// which gate covers it.
    ///
    /// Kept as an inventory rather than a one-sided assertion because the
    /// workspace has **two** egress gates and always did: this consent seam, for
    /// web research and skill discovery; and the tool permission system, for the
    /// `fetch` tool, which declares `Effect::Network` and is asked about through
    /// the permission profile. Both ask a person. Neither covers the other.
    ///
    /// The rest are not user-directed reach at all: inference (loopback-only,
    /// ADR-0164), credential exchange the user initiated by logging in, and the
    /// updater.
    const SANCTIONED_EGRESS: &[(&str, &str)] = &[
        ("localpilot-cli/src/research.rs", "consent seam"),
        ("localpilot-cli/src/skill_discovery.rs", "consent seam"),
        (
            "localpilot-tools/src/builtins.rs",
            "tool permission (Effect::Network)",
        ),
        (
            "localpilot-llm-openai/src/lib.rs",
            "inference, loopback-only",
        ),
        (
            "localpilot-llm-anthropic/src/lib.rs",
            "inference, loopback-only",
        ),
        (
            "localpilot-llm/src/discovery.rs",
            "inference, loopback-only",
        ),
        (
            "localpilot-llm-core/src/auth.rs",
            "credential exchange the user started",
        ),
        (
            "localpilot-cli/src/login_cmd.rs",
            "credential exchange the user started",
        ),
        ("localpilot-dist/src/install.rs", "updater"),
        ("localpilot-stack/src/lib.rs", "updater"),
        ("localpilot-cli/src/update.rs", "updater"),
    ];

    #[test]
    fn no_outbound_http_client_appears_outside_the_known_inventory() {
        // A grep test, and worth the ugliness. A gate is only a gate if there is
        // no way around it, and the way around it is a client built somewhere
        // nobody thought to look. This fails when a new one appears — which a
        // reviewer reading a diff would not reliably catch — and the fix is
        // either to route it through a gate or to add it here with the gate that
        // covers it named out loud.
        ...
        while let Some(dir) = stack.pop() {
            ...
            for entry in entries.flatten() {
                ...
                if !text.contains("reqwest::Client::") {
                    continue;
                }
                let relative = path
                    .strip_prefix(&crates)
                    ...
                    .to_string_lossy()
                    .replace('\\', "/");
                // A test building its own client is testing something, not
                // reaching out on the user's behalf.
        ...
        let known: std::collections::BTreeSet<String> = SANCTIONED_EGRESS
            .iter()
        ...
        assert!(
            unexpected.is_empty(),
            "an outbound HTTP client with no gate named for it: {unexpected:?}. \
             Route it through a gate, or add it to SANCTIONED_EGRESS saying which \
             gate covers it."
        );

        // And the other direction, so the inventory cannot rot into a list of
        // places that no longer exist.
        let stale: Vec<&String> = known.difference(&found).collect();
        assert!(
            stale.is_empty(),
            "SANCTIONED_EGRESS lists a file that no longer builds a client: {stale:?}"
        );
    }
    ...
    fn web_source(
        ...
        web_source_with(server_url, allowlist, enabled, audit_log, None)
    }

    /// An approval that answers from a script and counts the asking, so an
    /// end-to-end run can be driven through approve / deny / allow-all without
    /// a terminal and without a single outbound request leaving the test.
    struct ScriptedApproval {
        answers: Vec<localpilot_research::ConsentAnswer>,
        asked: Arc<std::sync::atomic::AtomicUsize>,
    }

    impl localpilot_research::EgressApproval for ScriptedApproval {
        fn ask(&mut self, _request: &EgressRequest<'_>) -> localpilot_research::ConsentAnswer {
            self.asked
                .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            if self.answers.is_empty() {
                localpilot_research::ConsentAnswer::Deny
            } else {
    ...
    fn web_source_with(
        ...
        enabled: bool,
        audit_log: PathBuf,
        approval: Option<localpilot_research::SharedApproval>,
    ) -> (WebSource, Arc<FakeProvider>) {
        let mut access = WebAccess::new(enabled, allowlist, Vec::new());
        access.grant_session();
        if let Some(approval) = approval {
            access = access.with_approval(approval);
        }
    ...
    fn a_destination_on_the_referring_host_inherits_its_permission() {
        ...
        assert!(
            matches!(
                decide_destination(&access, &internal, Some("127.0.0.1"), "a test"),
        ...
        assert!(
            matches!(
                decide_destination(&access, &internal, Some("docs.example.com"), "a test"),
                DestinationVerdict::BlockedInternal
            ),
        ...
        assert!(
            matches!(
                decide_destination(&access, &internal, None, "a test"),
                DestinationVerdict::BlockedInternal
            ),
        ...
        assert!(
            matches!(
                decide_destination(&access, &scheme, Some("127.0.0.1"), "a test"),
                DestinationVerdict::BlockedScheme
            ),
    ...
    #[tokio::test]
    async fn disclosure_names_the_default_consent_posture_and_off_switches() {
        ...
        assert!(
            text.contains("you will be asked before each new host"),
            "states the reach, which is per-host consent rather than open web: {text}"
        );
        assert!(
    ...
    #[tokio::test]
    async fn disclosure_says_an_empty_allowlist_means_being_asked_not_being_stuck() {
        // An empty allowlist used to mean "nothing will be fetched this run",
        // and that is no longer what happens: it means every host is asked
        // about. Saying the old thing would now be actively misleading, because
        // a person reading it would think the run was inert.
        let dir = tempfile::tempdir().unwrap();
        let mut config = localpilot_config::Config::default();
        ...
        assert!(
            text.contains("you will be asked before each new host"),
            "the banner says the run asks rather than that it fetches nothing: {text}"
        );
        assert!(
            text.contains("with no terminal"),
            "and says what happens when there is nobody to ask: {text}"
        );
    }
