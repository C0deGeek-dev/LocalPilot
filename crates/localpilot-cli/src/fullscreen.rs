...
fn localmind_graph_lines(
    ...
    match result {
        Ok(summary) if summary.files == 0 && summary.symbols == 0 => vec![
            "No code graph has been indexed for this LocalMind store.".to_string(),
            "Saved sessions update it automatically when learning is enabled.".to_string(),
            "Press r to reindex now (unavailable in incognito or with learning disabled)."
                .to_string(),
        ],
        Ok(summary) => {
            let mut lines = vec![format!(
        ...
        Event::Key(key) if is_key_action(key) => {
            ...
            match app.handle_input(action, hit_map.editor_width) {
                ...
                AppCommand::LocalMindReindex => {
                    apply_pair_notice(
                        app,
                        "LocalMind graph reindex is available only in full-screen chat.",
                    );
                    PairHostAction::None
                }
...
async fn localmind_write_allowed(
...
async fn perform_localmind_review(
    ...
    if !localmind_write_allowed(&permissions, approver, &request).await {
...
const LOCALMIND_REINDEX_BATCH_LIMIT: usize = 8;
const LOCALMIND_REINDEX_INCOGNITO_REFUSAL: &str =
    "not available in incognito — a LocalMind graph reindex would write to the persistent knowledge store.";
const LOCALMIND_REINDEX_NO_STORE_REFUSAL: &str =
    "LocalMind graph reindex is unavailable because this workspace has no store.";

#[derive(Debug, PartialEq, Eq)]
enum LocalMindReindexPreflight {
    Ready(PathBuf),
    Incognito,
    MissingStore,
}

fn localmind_reindex_preflight(incognito: bool, cwd: &Path) -> LocalMindReindexPreflight {
    localmind_reindex_preflight_at(incognito, localpilot_localmind::resolve_store_root(cwd))
}

fn localmind_reindex_preflight_at(
    incognito: bool,
    store_root: localpilot_localmind::StoreRoot,
) -> LocalMindReindexPreflight {
    if incognito {
        return LocalMindReindexPreflight::Incognito;
    }
    match store_root {
        localpilot_localmind::StoreRoot::Found(root) => LocalMindReindexPreflight::Ready(root),
        localpilot_localmind::StoreRoot::NotFound(_) => LocalMindReindexPreflight::MissingStore,
    }
}

fn localmind_reindex_request(trusted: bool) -> PermissionRequest {
    PermissionRequest {
        tool: "localmind_graph_reindex".to_string(),
        effect: Effect::WritePath {
            inside_workspace: true,
            overwrite: false,
            secret_like: false,
        ...
        interactivity: Interactivity::Interactive,
        trusted,
        detail: "reindex the LocalMind code graph for this workspace".to_string(),
    }
}

#[derive(Debug)]
enum LocalMindReindexOutcome {
    Updated(localpilot_localmind::CodeGraphSummary),
    Cancelled(localpilot_localmind::CodeGraphSummary),
    Denied,
    Failed(String),
}

async fn perform_localmind_reindex(
    permissions: PermissionEngineHandle,
    approver: &dyn Approver,
    request: PermissionRequest,
    root: PathBuf,
    cancel: CancellationToken,
    progress_tx: mpsc::UnboundedSender<localpilot_localmind::CodeGraphSummary>,
) -> LocalMindReindexOutcome {
    if !localmind_write_allowed(&permissions, approver, &request).await {
        return LocalMindReindexOutcome::Denied;
    }

    let worker_cancel = cancel.clone();
    let handle = tokio::task::spawn_blocking(move || {
        localpilot_localmind::codegraph_reindex_all(
            &root,
            LOCALMIND_REINDEX_BATCH_LIMIT,
    ...
    guard.disarm();
    match result {
        Ok(Ok(summary)) if cancel.is_cancelled() && summary.remaining > 0 => {
            LocalMindReindexOutcome::Cancelled(summary)
        }
        Ok(Ok(summary)) => LocalMindReindexOutcome::Updated(summary),
        Ok(Err(error)) => LocalMindReindexOutcome::Failed(error.to_string()),
        Err(error) => LocalMindReindexOutcome::Failed(format!(
            "LocalMind graph worker did not complete: {error}"
        )),
    }
}

fn localmind_reindex_progress_notice(summary: &localpilot_localmind::CodeGraphSummary) -> String {
    let processed = summary.reindexed.saturating_add(summary.pruned);
    format!(
        "LocalMind graph reindex: {processed} changed file(s) processed · {} remaining",
        summary.remaining
    )
}

fn localmind_reindex_result_notice(
    label: &str,
    summary: &localpilot_localmind::CodeGraphSummary,
) -> String {
    format!(
        "LocalMind graph reindex {label}: {} reindexed · {} pruned · {} unchanged · {} rejected · {} remaining",
        summary.reindexed,
        summary.pruned,
        summary.unchanged,
        summary.rejected,
...
fn finish_localmind_reindex(
    app: &mut AppModel,
    refresh_root: &Path,
    outcome: LocalMindReindexOutcome,
) {
    match outcome {
        LocalMindReindexOutcome::Updated(summary) => {
            app.refresh_localmind(load_localmind_data(refresh_root));
            app.apply_runtime(RuntimeUpdate::Notice(localmind_reindex_result_notice(
                "complete", &summary,
            )));
        }
        LocalMindReindexOutcome::Cancelled(summary) => {
            app.refresh_localmind(load_localmind_data(refresh_root));
            app.apply_runtime(RuntimeUpdate::Warning(localmind_reindex_result_notice(
                "cancelled",
                &summary,
            )));
        }
        LocalMindReindexOutcome::Denied => app.apply_runtime(RuntimeUpdate::Warning(
            "LocalMind graph reindex denied; no changes were made.".to_string(),
        )),
        LocalMindReindexOutcome::Failed(error) => {
            app.refresh_localmind(load_localmind_data(refresh_root));
            app.apply_runtime(RuntimeUpdate::Warning(format!(
                "LocalMind graph reindex failed: {error}"
            )));
        }
    }
    app.apply_runtime(RuntimeUpdate::Stopped(StopState::Done));
...
async fn drive_localmind_reindex(
    terminal: &mut Terminal<CrosstermBackend<Stdout>>,
    app: &mut AppModel,
    runtime: &mut SessionRuntime,
    ctx: SlashContext<'_>,
    ...
    let root = match localmind_reindex_preflight(runtime.is_incognito(), ctx.cwd) {
        LocalMindReindexPreflight::Ready(root) => root,
        LocalMindReindexPreflight::Incognito => {
            app.apply_runtime(RuntimeUpdate::Warning(
                LOCALMIND_REINDEX_INCOGNITO_REFUSAL.to_string(),
            ));
            return Ok(false);
        }
        LocalMindReindexPreflight::MissingStore => {
            app.apply_runtime(RuntimeUpdate::Warning(
                LOCALMIND_REINDEX_NO_STORE_REFUSAL.to_string(),
            ));
            return Ok(false);
        }
    };
    ...
    let permissions = runtime.permission_engine_handle();
    let request = localmind_reindex_request(runtime.trusted());
    let approver = TuiApprover::new(approval_tx.clone());
    let refresh_root = ctx.cwd.to_path_buf();
    let cancel = CancellationToken::new();
    let (progress_tx, mut progress_rx) = mpsc::unbounded_channel();
    let operation = perform_localmind_reindex(
        permissions,
        &approver,
        request,
        root,
    ...
    let mut progress = |app: &mut AppModel| {
        while let Ok(summary) = progress_rx.try_recv() {
            app.apply_runtime(RuntimeUpdate::Notice(localmind_reindex_progress_notice(
                &summary,
            )));
        }
    };
    ...
    drive_fullscreen_operation(
        ...
        operation,
        move |app: &mut AppModel, outcome| {
            finish_localmind_reindex(app, &refresh_root, outcome);
        },
    )
    .await
}
        ...
        loop {
            tokio::select! {
                            ...
                            OperationInputOutcome::Exit => {
                                ...
                                if kind.waits_for_durable_boundary() {
                                    if !durable_wait_notified {
                                        if let Some(notice) = kind.wait_notice(true) {
                                            app.apply_runtime(RuntimeUpdate::Warning(
                                                notice.to_string(),
                                            ));
                                        }
                                    ...
                                    if kind.clears_cancellation_request() {
                ...
                _ = frame_tick.tick() => {
                    ...
                    {
                        if let Some(notice) = kind.wait_notice(false) {
                            app.apply_runtime(RuntimeUpdate::Warning(notice.to_string()));
                        }
                        if kind.clears_cancellation_request() {
                            app.clear_cancellation_request();
                        }
    ...
    match event {
        Event::Key(key) if is_key_action(key) => {
            ...
            match command {
                ...
                AppCommand::LocalMindReindex => {
                    app.apply_runtime(RuntimeUpdate::Notice(
                        "LocalMind graph reindex is available when the current operation is idle."
                            .to_string(),
                    ));
                    false
                }
...
mod tests {
    ...
    #[test]
    fn graph_empty_state_names_the_real_panel_action_not_a_dead_cli_route() {
        let lines = localmind_graph_lines(Ok(Default::default()));
        assert_eq!(
            lines,
            vec![
                "No code graph has been indexed for this LocalMind store.",
                "Saved sessions update it automatically when learning is enabled.",
                "Press r to reindex now (unavailable in incognito or with learning disabled).",
            ]
        );
        let rendered = lines.join("\n");
        assert!(!rendered.contains("localpilot learning graph reindex"));
        assert!(
    ...
    #[test]
    fn graph_reindex_preflight_refuses_incognito_and_missing_store_without_writes() {
        let dir = tempfile::tempdir().expect("temporary workspace");
        assert_eq!(
            localmind_reindex_preflight_at(
                true,
                localpilot_localmind::StoreRoot::NotFound(dir.path().to_path_buf()),
            ),
            LocalMindReindexPreflight::Incognito
        );
        assert!(LOCALMIND_REINDEX_INCOGNITO_REFUSAL.starts_with("not available in incognito"));
        assert!(LOCALMIND_REINDEX_INCOGNITO_REFUSAL.contains("persistent knowledge store"));
        assert_eq!(
            localmind_reindex_preflight_at(
                false,
                localpilot_localmind::StoreRoot::NotFound(dir.path().to_path_buf()),
        ...
        assert!(LOCALMIND_REINDEX_NO_STORE_REFUSAL.contains("no store"));
        assert!(!dir.path().join(".localmind").exists());
        assert!(!dir.path().join(".localmind.toml").exists());
    }

    #[tokio::test]
    async fn graph_reindex_permission_obeys_allow_ask_and_denial_without_writing() {
        let dir = tempfile::tempdir().expect("temporary workspace");
        std::fs::write(
            dir.path().join(".localmind.toml"),
            "[learning]\nenabled = true\n",
        )
        .expect("LocalMind config");
        let request = localmind_reindex_request(false);
        assert!(matches!(
            request.effect,
            Effect::WritePath {
                inside_workspace: true,
        ...
        assert_eq!(permissions.snapshot().decide(&request), Decision::Ask);

        let denied = localpilot_sandbox::ScriptedApprover::new(vec![false]);
        let (progress_tx, _progress_rx) = mpsc::unbounded_channel();
        let outcome = perform_localmind_reindex(
            permissions,
            &denied,
            request,
        ...
        .await;
        assert!(matches!(outcome, LocalMindReindexOutcome::Denied));
        assert!(!dir.path().join(".localmind").exists());

        let bypass = PermissionEngineHandle::new(localpilot_sandbox::PermissionEngine::new(
            Profile::Bypass,
            Vec::new(),
        ));
        let allow_request = localmind_reindex_request(false);
        assert_eq!(bypass.snapshot().decide(&allow_request), Decision::Allow);
        let unused_approver = localpilot_sandbox::ScriptedApprover::new(Vec::new());
        assert!(localmind_write_allowed(&bypass, &unused_approver, &allow_request).await);
    }

    #[tokio::test]
    async fn graph_reindex_populates_and_refreshes_the_open_graph_panel_with_progress() {
        let dir = tempfile::tempdir().expect("temporary workspace");
        let root = dir.path();
        ...
        let mut app = app();
        app.open_localmind(load_localmind_data(root));
        ...
        let approver = localpilot_sandbox::ScriptedApprover::new(Vec::new());
        let (progress_tx, mut progress_rx) = mpsc::unbounded_channel();
        let outcome = perform_localmind_reindex(
            permissions,
            &approver,
            localmind_reindex_request(true),
            root.to_path_buf(),
            CancellationToken::new(),
            progress_tx,
        )
        ...
        while let Ok(summary) = progress_rx.try_recv() {
            remaining.push(summary.remaining);
        }
        assert_eq!(remaining, vec![9, 1, 0]);
        finish_localmind_reindex(&mut app, root, outcome);

        assert_eq!(
            app.localmind_section(),
            Some(localpilot_terminal_ui::LocalMindSection::Graph),
            "refresh must not leave or reset the open Graph panel"
        );
        let screen = rendered_screen(&app);
        assert!(
            screen.contains("9 files"),
            "refreshed graph missing: {screen}"
        );
        assert!(
            screen.contains("rust"),
            "language summary missing: {screen}"
        );
    }

    #[tokio::test]
    async fn graph_reindex_honours_learning_disabled_without_creating_state() {
        let dir = tempfile::tempdir().expect("temporary workspace");
        std::fs::write(
            dir.path().join(".localmind.toml"),
            "[learning]\nenabled = false\n",
        )
        .expect("disabled LocalMind config");
        let permissions = PermissionEngineHandle::new(localpilot_sandbox::PermissionEngine::new(
            Profile::Bypass,
            Vec::new(),
        ));
        ...
        let outcome = perform_localmind_reindex(
            permissions,
            &approver,
            localmind_reindex_request(false),
            dir.path().to_path_buf(),
            CancellationToken::new(),
            progress_tx,
        )
        .await;
        assert!(matches!(outcome, LocalMindReindexOutcome::Failed(_)));
        assert!(!dir.path().join(".localmind").exists());
    }

    #[test]
    fn graph_reindex_notices_surface_partial_remaining_work() {
        let summary = localpilot_localmind::CodeGraphSummary {
            reindexed: 8,
            remaining: 3,
            ..Default::default()
        };
        assert!(localmind_reindex_progress_notice(&summary).contains("3 remaining"));
        let cancelled = localmind_reindex_result_notice("cancelled", &summary);
        assert!(cancelled.contains("8 reindexed"));
        assert!(cancelled.contains("3 remaining"));
    }
...
"#,
    ...
    async fn review_permission_uses_the_interactive_approver_when_asked() {
        ...
        assert!(!localmind_write_allowed(&permissions, &deny, &request).await);
        let allow = localpilot_sandbox::ScriptedApprover::new(vec![true]);
        assert!(localmind_write_allowed(&permissions, &allow, &request).await);
    }
