...
pub fn freshness(
    ...
    writeln!(
        out,
        "freshness pass{mode}: scanned {}, flagged {} (low-quality {}, version-sensitive {}, never-retrieved {}, age {}){cap}",
        outcome.scanned,
        outcome.flagged.len(),
        outcome.low_quality,
...
/// Clear the staleness flag on specific memories — the reviewer's "keep this".
///
/// The other half of `freshness --apply`. A flagged memory is excluded from the
/// queryless context primer, so until this existed a false positive silently
/// stopped being offered as background context with no way to restore it.
///
/// # Errors
/// Returns an error when the store cannot be opened or written.
pub fn keep(
    project_root: &Path,
    memory_ids: &[String],
    format: OutputFormat,
    ...
    let outcome = localpilot_localmind::clear_stale(project_root, memory_ids)?;
    if matches!(format, OutputFormat::Json) {
        writeln!(out, "{}", serde_json::to_string_pretty(&outcome)?)?;
        return Ok(());
    }
    writeln!(out, "kept: {}", outcome.cleared.len())?;
    for id in &outcome.cleared {
        writeln!(out, "  {id}")?;
    }
    // Reported rather than folded into the count: an id that was not flagged,
    // or does not exist, looks exactly like success if it is only counted.
    if !outcome.untouched.is_empty() {
        writeln!(
            out,
            "not flagged (nothing to clear): {}",
            outcome.untouched.len()
        )?;
        for id in &outcome.untouched {
            writeln!(out, "  {id}")?;
        }
    }
    Ok(())
...
pub fn lifecycle(
    ...
    // The never-retrieved count is the one number here that invites a decision —
    // and it is the one that cannot carry one on its own. Every injection touches
    // at most one distinct memory, so a corpus that has seen fewer injections
    // than it has memories has a hard floor of entries that *cannot* have been
    // retrieved. Printing the count without that floor reads as a verdict on the
    // memories rather than a fact about how few turns have run.
    lifecycle_section(out, "never retrieved", &listing.never_retrieved)?;
    if !listing.never_retrieved.is_empty() {
        writeln!(
            out,
            "  {} injection(s) have ever happened across {} memories \
             (at least {} retrieving turns).",
            listing.total_injections,
            listing.total,
            listing.implied_turns()
        )?;
        if listing.unreachable_floor > 0 {
            writeln!(
                out,
                "  At least {} memories could not have been retrieved whatever their quality.",
                listing.unreachable_floor
            )?;
        }
        if listing.never_retrieved_is_informative() {
            writeln!(
                out,
                "  The count exceeds that floor, so it carries some signal — but it is a \
                 cue to review, never to delete."
            )?;
        } else {
            writeln!(
                out,
                "  That accounts for most of this list. **Not dead weight — not yet measured.** \
                 The signal is reporting how few turns have run, not which memories are unused."
            )?;
        }
    }
...
pub fn revalidate(
    ...
    // The operator-driven path: prepare a check for one memory and send nothing.
    //
    // Deriving a query and sending it automatically was built, measured against a
    // real store, and not shipped: of 475 memories, 48 read as candidates, 9
    // produced a query at all, and about three were questions anybody could act
    // on. A pass reaching three memories does not earn an outbound request. So
    // the machine proposes and a person decides.
    if let Some(memory_id) = memory {
        let Some(proposal) = learning::revalidation_proposal(cwd, memory_id)? else {
            writeln!(out, "no accepted memory with id `{memory_id}`")?;
            return Ok(());
        };
        if format == OutputFormat::Json {
        ...
        writeln!(out, "memory {}", proposal.memory_id)?;
        writeln!(out, "  {}", proposal.body.trim())?;
        writeln!(out)?;
        writeln!(
            out,
            "  offline heuristic: {}",
            if proposal.is_candidate {
                "worth re-checking (names a version-sensitive marker)"
            } else {
                "not selected — the heuristic sees nothing version-sensitive here"
            }
        )?;
        match &proposal.proposed_query {
            Some(query) => {
                writeln!(out, "  proposed query: {query}")?;
                writeln!(
                    out,
                    "  Built only from third-party dependencies this workspace"
                )?;
                writeln!(
                    out,
                    "  declares, so nothing internal is in it. Edit it before"
                )?;
                writeln!(out, "  using it — it is a starting point, not a decision.")?;
            }
            None => {
                writeln!(
                    out,
                    "  proposed query: none — nothing in this memory can be"
                )?;
                writeln!(out, "  shown to be public already, so nothing is proposed.")?;
            }
        }
        writeln!(out)?;
        writeln!(
            out,
            "  Nothing has been sent. To check it against the world, run"
        )?;
        writeln!(
            out,
            "  `localpilot research \"<your question>\"` — that path asks before"
        )?;
        writeln!(
            out,
            "  every request, audits each one, and routes findings to review."
        )?;
        return Ok(());
    }
    if apply {
...
mod tests {
    ...
    #[test]
    fn a_single_memory_proposal_sends_nothing_and_proposes_nothing_it_cannot_vouch_for() {
        // The operator-driven path. The machine prepares the question; a person
        // decides whether any of it leaves.
        let dir = tempfile::tempdir().unwrap();
        seed_accepted(dir.path(), "the --foo flag was deprecated in v1.2");
        let id = learning::memory_lifecycle(dir.path(), 1)
            .unwrap()
            .never_retrieved
            .first()
        ...
        // A workspace with no declared dependencies can vouch for no name, so the
        // proposal is a refusal rather than a guess.
        assert!(text.contains("proposed query: none"), "{text}");
    }
