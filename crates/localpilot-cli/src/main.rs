...
struct Cli {
    /// Approve everything for this run: every tool permission request and every
    /// outbound request, without asking.
    ///
    /// Tools run without prompting, including edits outside this workspace and
    /// shell commands. Web requests leave the machine without asking, to any
    /// host not explicitly disallowed. Never persisted and never read from
    /// config — pass it again next time or the prompts come back.
    ///
    /// `[research.web] enabled = false` and `[research.web] disallowlist` still
    /// hold: this answers questions, it does not lift a refusal already written
    /// down.
    #[arg(
...
enum Command {
    ...
    ServerServe,
    /// Internal: wait for shared embedding leases to drain, then stop the exact
    /// LocalPilot-owned server. Spawned only by the lifecycle guard.
    #[command(name = "__embed-reap", hide = true)]
    EmbedReap {
        #[arg(long)]
        endpoint: String,
        #[arg(long)]
...
enum McpCommand {
    ...
    Serve {
        ...
        #[arg(long)]
        no_approvals: bool,
        /// Exit after this many minutes with no client message. `0` waits
        /// forever. Defaults to 240 (four hours): end of input already ends a
        /// server whose client exited, but a client that is *abandoned* keeps
        /// the pipe open, and such a server can hold the binary open for days.
...
enum LearningCommand {
    ...
    Audit,
    /// Keep flagged memory: clear the staleness flag set by `freshness --apply`.
    ///
    /// The reviewer's "no, this one is fine" decision. A flagged memory is
    /// excluded from the queryless context primer, so a false positive stops
    /// being offered as background context until this lifts it.
    Keep {
        /// Memory ids to keep. Accepts project and global ids.
        #[arg(required = true)]
    ...
    /// One read-only curation pass: what is in the store, what the signals can and
    /// cannot tell you, and which memories are worth a look. Applies nothing.
    Curate,
    Revalidate {
        /// Prepare a check for one memory and send nothing: print the lesson, whether
        /// the offline heuristic selects it, and a proposed query built only from
        /// third-party dependencies this workspace declares. The operator decides
        /// what — if anything — leaves.
...
async fn run() -> anyhow::Result<std::process::ExitCode> {
    ...
    // One decision, made once, covering everything this process subsequently
    // does. Announced before anything runs, so the person who typed it sees what
    // they turned off while there is still time to press ctrl-c.
    if cli.dangerously_skip_permissions {
        bypass::engage();
        bypass::announce(&mut std::io::stderr());
    }
    ...
    // Bring the embedding server up for the life of this run, when the project
    // configured one and this command actually runs a session. Nothing starts at
    // boot: a 640 MB model server appearing at login is exactly the unexpected
    // process a local-first tool should not install. Nothing starts for
    // `--version`, `doctor`, or any other command that never retrieves, either.
    // The guard, rather than one particular close-out branch, owns release: it
    // therefore covers ordinary return, propagated errors, cancellation, and
    // unwinding. If a standalone LocalMind client is still leased at drop, a
    // detached LocalPilot reaper owns the eventual last-lease stop.
    let _embedding_lifecycle = starts_a_session(&command)
        .then(embed_lifecycle::ensure_for_run)
        .flatten();
    ...
    match command {
        ...
        Command::Mcp(McpCommand::Serve {
            ...
            rpc_cmd::run(
                ...
                rpc_cmd::WireProtocol::Mcp {
                    ...
                    idle_timeout: session_cmd::resolve_idle_timeout(idle_timeout),
                },
                resume,
            )
            .await?;
        ...
        Command::EmbedReap {
            endpoint,
            server_pid,
        } => embed_lifecycle::reap(&endpoint, server_pid),
        ...
        Command::Learning { workspace, command } => {
            ...
            match command {
                ...
                LearningCommand::Keep {
                    ...
                    let is_tty = io::stdout().is_terminal();
                    let resolved = output::resolve_format(format, json, is_tty);
                    learning_cmd::keep(root, &memory_ids, resolved, &mut stdout)?;
                }
                ...
                LearningCommand::Curate => {
                    curate_cmd::curate(root, &mut stdout)?;
                }
        ...
        Command::Skills { command } => {
            let cwd = std::env::current_dir()?;
            let mut stdout = io::stdout().lock();
            let style = skill_output::CliSkillCatalogStyle::from_environment(stdout.is_terminal());
            ...
            let outcome = match command {
                ...
                other => {
                    let stdin_is_tty = io::stdin().is_terminal();
                    skills_cmd::run(other, &cwd, stdin_is_tty, &style, &mut stdout)?
                }
            };
            stdout.flush()?;
            if outcome.had_failure {
...
/// Does this command run a session that retrieves memory?
///
/// The gate on starting the embedding server. Listed positively rather than as
/// an exclusion, so a new command defaults to *not* launching a model server —
/// the safe direction for something that costs 640 MB of the user's machine.
fn starts_a_session(command: &Command) -> bool {
    #[cfg(feature = "tui")]
    if matches!(command, Command::Chat { .. } | Command::Pair(_)) {
        return true;
    }
    matches!(
        command,
    ...
    #[cfg(feature = "tui")]
    {
            ...
            localpilot_config::load(&ConfigPaths::standard(&cwd), &CliOverrides::default())
        {
            if config.resolve_model(None).is_some() {
                ...
                // A configured bare invocation is the same long-lived chat
                // session as `localpilot chat`; keep its owner lease alive for
                // the complete REPL lifetime as well.
                let _embedding_lifecycle = embed_lifecycle::ensure_for_run();
