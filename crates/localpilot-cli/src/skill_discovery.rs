...
use localpilot_research::{AuditEntry, EgressOutcome, EgressRequest, WebAccess};
use localpilot_skills::{
    rank, source_id, validate_repo, DiscoveredSkill, GitFetcher, MatchState, ProposalState,
    ProposalStore, Ranked, ReadScope, RepoFetcher, SkillCatalogStyle, SkillProposal, SkillsManager,
    ValidatedRepo,
};
...
pub async fn discover_web_repositories(
    ...
    match cfg.access.decide(&EgressRequest {
        host: search_host,
        url: search_host,
        purpose: &format!("searching for a skill: {query}"),
    }) {
        EgressOutcome::Fetch => {}
        EgressOutcome::Skip => {
            audit(&cfg.audit_log, search_host, search_host, "skipped", query);
            result.notes.push(format!(
                "search host `{search_host}` was not approved — skipped"
            ));
            return result;
        }
        EgressOutcome::Off => {
    ...
    for url in search_result.urls.into_iter().take(cfg.limit) {
        ...
        match cfg.access.decide(&EgressRequest {
            host: &host,
            url: &url,
            purpose: &format!("checking a candidate skill repository for: {query}"),
        }) {
            EgressOutcome::Fetch => {}
            _ => {
                audit(&cfg.audit_log, &url, &host, "skipped", query);
                result.notes.push(format!(
                    "candidate host `{host}` was not approved — skipped"
                ));
                continue;
            }
        }
...
async fn run_discovery(
    ...
    if web_on {
        ...
        // Same launch flag, same seam: a run that pre-approved everything is not
        // asked here either. With no flag and no terminal, the default refusal
        // stands and discovery reaches only pre-approved hosts.
    ...
    render_discovery(query, &ranked, deps.style, out)?;
...
fn render_discovery(
    ...
    for (index, m) in relevant.iter().take(10).enumerate() {
        ...
        writeln!(
            out,
            "- [{}] {}{}",
            style.state(m.skill.state),
            style.name(&m.skill.name),
            origin,
        )?;
        // This is the decision report, not the lean model-facing index: retain
        // the complete trigger-bearing description and make it its own line.
        writeln!(out, "  {}", m.skill.description)?;
    }
...
mod tests {
    ...
    fn render_discovery_shows_only_relevant_matches_and_the_recommendation() {
        ...
        render_discovery(
            "threejs materials",
            &ranked,
            &localpilot_skills::PlainSkillCatalogStyle,
            &mut buf,
        )
        .unwrap();
        ...
        assert!(
            ...
            "the full description is visually bounded from the recommendation: {text}"
        );
        assert!(
    ...
    async fn end_to_end_threejs_discovers_recommends_saves_and_mutates_nothing() {
        ...
        let deps = DiscoveryDeps {
            ...
            style: &localpilot_skills::PlainSkillCatalogStyle,
        };
        let mut buf = Vec::new();
    ...
    async fn a_non_allowlisted_candidate_host_is_skipped() {
        ...
        assert!(found.notes.iter().any(|n| n.contains("was not approved")));
    }
}
