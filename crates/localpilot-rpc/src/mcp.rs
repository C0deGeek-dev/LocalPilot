...
pub struct McpServeOptions {
    ...
    pub approvals: bool,
    /// Give up after this long with no client message. `None` waits forever,
    /// which is the old behaviour.
    ///
    /// End of input already ends the loop, and for a client that exits that is
    /// enough. It is not enough for a client that is *abandoned*: the pipe stays
    /// open as long as the process holding it exists, so a stale editor session
    /// keeps its server alive indefinitely. Found on a real machine — an
    /// `mcp serve` still running two days later under a Claude Code process that
    /// its own self-update had replaced, holding the `localpilot` binary open
    /// and making every source install fail with `Access is denied`.
    ///
    /// The timer is generous by default because a legitimately idle session is
    /// normal — someone can leave an editor open over lunch — and it is reset by
    /// any client message and by a running turn, so a long turn is never cut off.
    ...
    W: AsyncWrite + Unpin,
{
    ...
    loop {
        let message = tokio::select! {
            ...
            () = idle_deadline(options.idle_timeout) => {
                // Nothing from the client for the whole window. A turn in
                // flight would have produced events and reset this, so there is
                // nothing running to cut off.
                break;
            }
...
/// Sleep until the idle window expires, or never when there is no window.
///
/// Pairs with the read in the same `select!`: any client message cancels this
/// future and the next loop iteration starts a fresh one, so the window means
/// "since the last thing the client said" without any bookkeeping.
async fn idle_deadline(window: Option<Duration>) {
    match window {
...
/// The default idle window: four hours.
///
/// Long enough that a session left open over a meal is untouched, short enough
/// that an abandoned one does not hold a binary open for days. Nothing about
/// four is principled — it is a compromise, and `--idle-timeout` exists because
/// somebody's workflow will disagree with it.
pub const DEFAULT_MCP_IDLE_TIMEOUT: Duration = Duration::from_secs(4 * 60 * 60);
