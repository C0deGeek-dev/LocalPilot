...
fn options(root: &std::path::Path, approvals: bool) -> McpServeOptions {
    McpServeOptions {
        ...
        approvals,
        // No idle window: these drive the loop to end of input deliberately, and
        // a timer would race the assertions.
        idle_timeout: None,
    }
}
...
/// An abandoned client keeps the pipe open, so end of input never comes. The
/// idle window is the only thing that ends such a server — found the hard way,
/// as an `mcp serve` still running two days later under a host process its own
/// self-update had replaced, holding the binary open so every install failed.
#[tokio::test]
async fn a_client_that_goes_silent_forever_does_not_keep_the_server_alive() {
    let (dir, mut runtime, ask_rx, registry) = build_full(FakeProvider::new());
    let mut opts = options(dir.path(), true);
    opts.idle_timeout = Some(std::time::Duration::from_millis(150));

    // The client half is held open and never written to: exactly an abandoned
    // host. Without the idle window this waits forever.
    let (client_io, server_io) = tokio::io::duplex(64 * 1024);
    let (server_read, server_write) = tokio::io::split(server_io);
    ...
    assert!(
        served.is_ok(),
        "the server must give up on its own; it was still waiting when the test timed out"
    );
}
