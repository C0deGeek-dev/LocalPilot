...
pub enum BadOutputKind {
    ...
    /// Read-only compatibility fallback for retired or newer diagnostic kinds.
    /// Live detectors never emit this; re-serialization normalizes it to
    /// `unknown` without changing the diagnostic's attempt, health, or actions.
    #[serde(other)]
    Unknown,
}
...
/// confirm a tool exists). Unlike [`NoProgressDetector`], it observes failures
/// rather than successful calls. It fires when the same `(tool, normalized error)`
...
/// failure breaker ([`RepeatedErrorBreaker`]) cannot see
...
/// dependency, like [`RepeatedErrorBreaker`].
#[derive(Debug, Clone)]
...
mod tests {
    ...
    // Repeat accounting is bounded to the observation window. All of these use a
