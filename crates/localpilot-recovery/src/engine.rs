...
impl RecoveryEngine {
    ...
    /// Select the payload-limit recovery rung for a request carrying multiple
    /// images. This is non-degrading: the provider rejected the payload shape,
    /// not the model's output, so consulting the rung does not spend the bad-turn
    /// repair budget or change model health.
    #[must_use]
    pub fn image_limit_rung(&self, image_count: usize) -> Option<RecoveryAction> {
        (image_count > 1).then_some(RecoveryAction::LowerImageCount)
    }
}
...
mod tests {
    ...
    #[test]
    fn multiple_images_emit_the_non_degrading_lower_image_count_rung() {
        let engine = RecoveryEngine::new(RecoveryBudget::default());
        assert_eq!(
            engine.image_limit_rung(3),
    ...
    #[test]
    fn text_only_and_single_image_payloads_do_not_emit_the_image_rung() {
        let engine = RecoveryEngine::new(RecoveryBudget::default());
        assert_eq!(engine.image_limit_rung(0), None);
        assert_eq!(engine.image_limit_rung(1), None);
    }
