...
impl CalibratingProvider {
    fn new() -> Self {
        let mut declaration = FakeProvider::new().declaration().clone();
        declaration.max_context_tokens = Some(12_000);
...
impl ModelProvider for CalibratingProvider {
    ...
    async fn stream(&self, request: ModelRequest) -> Result<ModelEventStream, ProviderError> {
        ...
        Ok(Box::pin(futures::stream::iter([
            Ok(ModelEvent::TextDelta(if call == 0 {
                "calibrated".to_string()
            } else {
                "continued after calibrated compaction".to_string()
            })),
...
async fn output_limit_stop_discards_partial_reply() {
    let mut declaration = FakeProvider::new().declaration().clone();
    declaration.max_context_tokens = Some(10_000);
    declaration.max_output_tokens = Some(2_000);
    let provider = FakeProvider::new()
...
#[tokio::test]
async fn context_exhaustion_compacts_without_appending_a_chunked_write_steer() {
    let mut declaration = FakeProvider::new().declaration().clone();
    declaration.max_context_tokens = Some(10_000);
    declaration.max_output_tokens = Some(2_000);
    let provider = Arc::new(
        FakeProvider::new()
            .with_declaration(declaration)
            .script(vec![
                Ok(ModelEvent::TextDelta("prior work recorded".to_string())),
                Ok(ModelEvent::Done),
            ])
            // Match the OpenAI-compatible ordering where the finish reason can
            // precede the usage object in the same final chunk. The harness must
            // defer the provisional output-cap warning until this usage arrives.
            .script(vec![
                Ok(ModelEvent::TextDelta("partial edit".to_string())),
                Ok(ModelEvent::OutputLimit {
                    message: "raise provider max_tokens and write smaller pieces".to_string(),
    ...
    let reason = h
        .runtime
        .run_turn("finish the edit", &h.events, &h.cancel)
        .await;

    assert_eq!(reason, StopReason::Done);
    assert_eq!(provider.requests().len(), 3, "one compacted retry");
    let events = drain(&mut rx);
    assert!(events.iter().any(|event| matches!(
        event,
        RuntimeEvent::Warning(message)
            if message.contains("9800 prompt")
                && message.contains("200 output")
                && message.contains("window 10000")
    )));
    assert!(events
        .iter()
    ...
    assert!(!events.iter().any(|event| matches!(
        event,
        RuntimeEvent::Warning(message)
            if message.contains("smaller pieces") || message.contains("raise provider max_tokens")
    )));

    let transcript = h.store.read_transcript(h.runtime.session_id()).unwrap();
    let persisted = serde_json::to_string(&transcript).unwrap();
    assert!(!persisted.contains("partial edit"));
    assert!(!persisted.contains("append_file"));
    assert!(persisted.contains("recovered after compaction"));
}

#[tokio::test]
async fn context_exhaustion_stops_honestly_when_history_is_already_minimal() {
    let mut declaration = FakeProvider::new().declaration().clone();
    declaration.max_context_tokens = Some(10_000);
    declaration.max_output_tokens = Some(2_000);
    let provider = Arc::new(
        FakeProvider::new()
            ...
            .script(vec![
                Ok(ModelEvent::OutputLimit {
                    message: "raise provider max_tokens".to_string(),
                    truncated_tools: vec!["write_file".to_string()],
                }),
                Ok(ModelEvent::Usage(TokenUsage {
                    input_tokens: 9_900,
    ...
    let warnings: Vec<_> = drain(&mut rx)
        .into_iter()
    ...
    assert!(warnings.iter().any(
        |message| message.contains("active history is already minimal")
            && message.contains("start a new session or split the task")
    ));
    assert!(!warnings
        .iter()
        .any(|message| message.contains("max_tokens") || message.contains("smaller pieces")));
}
...
#[tokio::test]
async fn provider_prompt_usage_calibrates_the_next_compaction_threshold() {
    let provider = Arc::new(CalibratingProvider::new());
    let mut h = build_from_provider(
        provider.clone(),
        SessionConfig {
    ...
    assert_eq!(requests.len(), 2);
    assert!(
        message_text(&requests[1].messages).contains("Conversation summary for trimmed history")
    );
    assert!(drain(&mut rx)
        .iter()
        .any(|event| matches!(event, RuntimeEvent::Compacted { .. })));
...
#[tokio::test]
async fn provider_prompt_usage_corrects_the_context_gauge() {
    let provider = FakeProvider::new().script(vec![
        ...
        Ok(ModelEvent::TextDelta("ok".to_string())),
        Ok(ModelEvent::Done),
    ]);
    let mut h = build(provider, &[], SessionConfig::default());
    let mut rx = h.events.subscribe();
    ...
    let corrected = drain(&mut rx)
        .into_iter()
        .filter_map(|event| match event {
            RuntimeEvent::ContextUsage { used, .. } => Some(used),
            _ => None,
        })
        .last();
    assert_eq!(corrected, Some(321));
}
...
async fn user_shell_runs_are_auditable_and_context_exclusion_works() {
    ...
    let excluded = h
        .runtime
        .run_user_shell_command_detailed("git --version", &cancel, true)
        .await
        .result;
    ...
    let included = h
        .runtime
        .run_user_shell_command_detailed("git --version", &cancel, false)
        .await
        .result;
...
async fn cancelled_user_shell_command_records_an_explicit_error_and_one_audit_pair() {
    ...
    let result = h
        .runtime
        .run_user_shell_command_detailed(command, &cancel, false)
        .await
        .result;
    assert!(result.is_error());
