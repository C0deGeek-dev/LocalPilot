...
#[test]
fn an_unverifiable_tool_result_is_recorded_as_unverified() {
    let directory = tempfile::tempdir().unwrap();
    let root = directory.path();
    #[cfg(windows)]
    let command = json!({ "program": "cmd", "args": ["/c", "exit 0"] });
    #[cfg(not(windows))]
    let command = json!({ "program": "sh", "args": ["-c", "exit 0"] });
    let provider = FakeProvider::new()
        .tool_call("c1", "run_shell", command)
    ...
    tokio::runtime::Runtime::new()
        .unwrap()
        .block_on(runtime.run_turn("run the command", &events, &cancel));

    let recorded = Store::open(root).read_events(session).unwrap();
    assert!(recorded.iter().any(|event| matches!(
        &event.kind,
        localpilot_store::SessionEventKind::ToolVerified { id, verdict }
            if id == "c1" && verdict == "unverified"
    )));
}
