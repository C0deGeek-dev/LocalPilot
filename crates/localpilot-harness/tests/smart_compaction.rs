...
#[tokio::test]
async fn a_multi_image_overflow_retries_with_only_the_newest_image() {
    let provider = Arc::new(
        FakeProvider::new()
            .script(vec![Err(ProviderError::InvalidRequest {
            ...
            .script(vec![Err(ProviderError::InvalidRequest {
                message: "compacted payload still too large".to_string(),
            })])
    ...
    let attachments = vec![
        ...
        ContentBlock::image("image/png", "newest"),
    ];

    let reason = h
        .runtime
        .run_turn_with_attachments("inspect these", &attachments, &h.events, &h.cancel)
        .await;

    assert_eq!(reason, StopReason::Done);
    let requests = provider.requests();
    assert_eq!(requests.len(), 3);
    let images = |request: &localpilot_llm::ModelRequest| {
        request
            .messages
            .iter()
    ...
    assert_eq!(images(&requests[0]), vec!["first", "second", "newest"]);
    assert_eq!(images(&requests[1]), vec!["first", "second", "newest"]);
    assert_eq!(images(&requests[2]), vec!["newest"]);
}
