...
use localpilot_llm::{
    InputBlockKind, ModelEvent, ModelEventStream, ModelProvider, ModelRequest, ProviderDeclaration,
    ProviderError, ProviderRegistry, QuotaInfo, ReasoningEffortHandle, ToolSpec,
};
...
use localpilot_store::{
    origin_for, transcript_from_events, OpenReason, SessionEventKind, Store, StoreError,
};
...
pub enum RuntimeEvent {
    ...
    /// Token usage.
    Usage(TokenUsage),
    /// Context usage for the current request. Emitted first from the calibrated
    /// local estimate before the request, then corrected to the provider's
    /// authoritative prompt count when usage arrives.
...
/// perfectly predict a later request with a different code/JSON/prose mix.
const CONTEXT_CALIBRATION_SAFETY_PERCENT: u128 = 5;
...
/// is a reasonable floor. Requests start with the bytes/4 heuristic, then
/// provider-reported prompt usage calibrates later compaction thresholds with a
/// small safety cushion — see docs/providers.md.
#[must_use]
...
/// Session-local correction for the bytes/4 context estimator. Providers count
/// the exact prompt they received; pairing that report with the estimate for the
/// same request gives the conversion needed for later compaction decisions.
/// Ratios below 1 are clamped so an over-counted prose turn never makes a later
/// dense-code request less safe than the raw estimator.
...
impl PromptTokenCalibration {
    fn observe(&mut self, estimated: usize, reported: u64) {
        if estimated == 0 || reported == 0 {
            return;
        }
        self.estimated = estimated;
        self.reported = reported.max(u64::try_from(estimated).unwrap_or(u64::MAX));
    }

    fn calibrated_estimate(self, estimated: usize) -> usize {
        if self.estimated == 0 {
            return estimated;
        }
        let numerator = u128::from(self.reported);
        let denominator = self.estimated as u128;
        let corrected = (estimated as u128)
            .saturating_mul(numerator)
            .saturating_add(denominator.saturating_sub(1))
            / denominator;
        usize::try_from(corrected).unwrap_or(usize::MAX)
    }

    /// Convert an authoritative-token budget into the raw estimator units used
    /// by the compactor, retaining a 5% cushion for content-mix drift.
    fn raw_budget(self, token_budget: usize) -> usize {
        let (numerator, denominator) = if self.estimated == 0 {
            (100 + CONTEXT_CALIBRATION_SAFETY_PERCENT, 100)
        } else {
            (
                u128::from(self.reported).saturating_mul(100 + CONTEXT_CALIBRATION_SAFETY_PERCENT),
                (self.estimated as u128).saturating_mul(100),
            )
        };
        let budget = (token_budget as u128).saturating_mul(denominator) / numerator;
        usize::try_from(budget).unwrap_or(usize::MAX)
    }
}

#[derive(Debug, Clone, Copy)]
struct ContextExhaustion {
    prompt_tokens: u64,
    output_tokens: u64,
    context_window: u64,
    output_cap: u64,
...
impl ContextExhaustion {
    fn classify(
        ...
        let output_cap = output_cap?;
        let prompt_tokens = usage.effective_input_tokens();
        let output_tokens = usage.output_tokens;
        let filled_window = prompt_tokens.saturating_add(output_tokens) >= context_window;
        let has_clear_output_headroom = output_tokens.saturating_mul(2) < output_cap;
        (prompt_tokens > 0 && filled_window && has_clear_output_headroom).then_some(Self {
            prompt_tokens,
            output_tokens,
            context_window,
            output_cap,
    ...
    fn detail(self) -> String {
        format!(
            "provider exhausted the context window ({} prompt + {} output = {} tokens; window {}, output cap {} was not reached)",
            self.prompt_tokens,
            self.output_tokens,
...
const PROVIDER_METADATA_FORMAT_VERSION: u32 = 1;

/// The inspectable provider snapshot stored under `.localpilot/providers/`.
/// It deliberately excludes credentials and per-response metadata: the live
/// provider declaration stays authoritative, while this record preserves the
/// capabilities a normal session actually opened with.
#[derive(Serialize)]
struct PersistedProviderMetadata<'a> {
    format_version: u32,
    declaration: &'a ProviderDeclaration,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum ProviderMetadataSync {
    Current,
    Updated,
}

fn sync_provider_metadata(
    store: &Store,
    declaration: &ProviderDeclaration,
) -> Result<ProviderMetadataSync, StoreError> {
    let serialized = serde_json::to_string(&PersistedProviderMetadata {
        format_version: PROVIDER_METADATA_FORMAT_VERSION,
        declaration,
    })?;
    // Compare the same redacted representation the store writes. Otherwise a
    // declaration string that resembles a secret would force a rewrite on
    // every session even though the durable record was already current.
    let metadata = serde_json::from_str(&redact(&serialized))?;

    match store.get_provider_metadata(&declaration.id) {
        Ok(Some(cached)) if cached == metadata => return Ok(ProviderMetadataSync::Current),
        Ok(_) => {}
        Err(error) => {
            tracing::warn!(
                provider = %declaration.id,
                error = %error,
                "failed to read provider metadata; replacing the stale cache"
            );
        }
    }

    store.put_provider_metadata(&declaration.id, &metadata)?;
    Ok(ProviderMetadataSync::Updated)
}
...
pub struct SessionRuntime {
    ...
    /// The second key is the already-calibrated raw-estimator budget, so a new
    /// provider usage sample invalidates an otherwise reusable projection.
    ...
    /// Registered context hooks and tool gates.
    hooks: HookFabric,
...
impl SessionRuntime {
    ...
    pub fn new(
        ...
        runtime.sync_active_provider_metadata();
    ...
    fn sync_active_provider_metadata(&self) {
        let declaration = self.provider.declaration();
        match sync_provider_metadata(&self.store, declaration) {
            Ok(ProviderMetadataSync::Current) => {
                tracing::trace!(provider = %declaration.id, "provider metadata cache is current");
            }
            Ok(ProviderMetadataSync::Updated) => {
                tracing::debug!(provider = %declaration.id, "provider metadata cache updated");
            }
            Err(error) => {
                tracing::warn!(
                    provider = %declaration.id,
                    error = %error,
                    "failed to persist provider metadata"
                );
            }
        }
    }
    ...
    /// permission path, returning typed redacted streams for a host that needs
    /// to present process status without parsing tool copy. The run always
    /// lands in the durable event log; unless `exclude_from_context` is set,
    /// the command and its output also enter the transcript as a
    /// [`Role::UserShell`] message. Cancellation produces an explicit error
    /// result and audit event; dropping an active `run_shell` dispatch reaps
    /// its process tree.
    ...
    pub fn set_active_provider(&mut self, id: &str) -> Result<SwitchOutcome, SwitchError> {
        ...
        self.compaction_cache = None;
        self.sync_active_provider_metadata();
        Ok(SwitchOutcome {
    ...
    /// The hook fabric, for registering context hooks and tool gates. Gates are
    /// tighten-only and run after the permission engine.
    ...
    /// Handle a provider overflow: first compact active history, then lower a
    /// still-oversized multi-image request to its newest image. Once the
    /// applicable rungs are exhausted, report a terminal failure.
    async fn try_overflow_retry(
        &mut self,
        compacted_retried: &mut bool,
        images_lowered: &mut bool,
        request_image_count: usize,
        request_image_limit: &mut Option<usize>,
        events: &broadcast::Sender<RuntimeEvent>,
        cancel: &CancellationToken,
    ) -> bool {
        if !*compacted_retried {
            *compacted_retried = true;
            let _ = events.send(RuntimeEvent::Warning(
        ...
        if !*images_lowered
            && self.recovery.image_limit_rung(request_image_count)
                == Some(RecoveryAction::LowerImageCount)
        {
            *images_lowered = true;
            *request_image_limit = Some(1);
            let _ = events.send(RuntimeEvent::Warning(format!(
                "provider still rejected the compacted request carrying {request_image_count} \
                 images as too large; retrying with only the newest image"
            )));
            return true;
        }
        let _ = events.send(RuntimeEvent::Warning(
    ...
    /// A provider can accept a request and still stop at the context boundary.
    /// This is not an output-cap fault: discard the partial response, compact
    /// active history, and retry once without appending a repair prompt.
    async fn try_context_exhaustion_retry(
        &mut self,
        retried: &mut bool,
        events: &broadcast::Sender<RuntimeEvent>,
        cancel: &CancellationToken,
        exhaustion: ContextExhaustion,
    ) -> bool {
        let detail = exhaustion.detail();
        if *retried {
            let _ = events.send(RuntimeEvent::Warning(format!(
                "{detail} again after compaction; start a new session or split the task"
            )));
            return false;
        }

        let Some((dropped_exchanges, context_used)) = self.shrink_for_overflow(cancel).await else {
            let _ = events.send(RuntimeEvent::Warning(format!(
                "{detail}; active history is already minimal, so start a new session or split the task"
            )));
            return false;
        };
        ...
        *retried = true;
        let _ = events.send(RuntimeEvent::Warning(format!(
            "{detail}; compacting and retrying once"
        )));
        let _ = events.send(RuntimeEvent::Compacted {
            dropped_exchanges,
            context_used,
...
{addition}"
    ...
    async fn compacted_history(
        ...
        if let Some((generation, cached_limit, cached)) = &self.compaction_cache {
            if *generation == self.history_generation && *cached_limit == token_limit {
                return (cached.clone(), false);
            }
        }
        let result = self.compact_candidate(token_limit, cancel).await;
        if result.compacted {
            if let Some(summary) = result.summary.clone() {
                self.record_event(SessionEventKind::Compacted { summary });
            }
        ...
        self.compaction_cache = Some((self.history_generation, token_limit, result.clone()));
    ...
    pub async fn run_turn_with_attachments(
        ...
        // retries with only the newest image before another overflow is terminal.
        let mut overflow_retried = false;
        let mut images_lowered = false;
        let mut request_image_limit = None;
        ...
        'turn: loop {
            ...
            let request_image_count = image_count(&request_messages);
            ...
            // The adapter only knows the wire stop reason. Hold its provisional
            // output-cap text until provider usage identifies the binding limit.
            let mut output_limit_message: Option<String> = None;
            ...
            // Keep this iteration separate from the turn-wide accumulator so a
            // limit diagnosis compares one request with its own context window.
                ...
                tokio::select! {
                    ...
                    event = stream.next() => match event {
                        ...
                        Some(Ok(ModelEvent::Usage(usage))) => {
                            request_usage.accumulate(usage);
                            ...
                            let reported_input = usage.effective_input_tokens();
                            if reported_input > 0 {
                                self.prompt_token_calibration
                                    .observe(request_estimate, reported_input);
                                let _ = events.send(RuntimeEvent::ContextUsage {
                                    used: usize::try_from(reported_input).unwrap_or(usize::MAX),
                                    limit: self.config.context_token_limit,
                                });
                            }
                        }
                        ...
                        Some(Ok(ModelEvent::OutputLimit {
                            ...
                            output_limit_message = Some(message);
                        }
            ...
            if output_limited {
                if let Some(exhaustion) = ContextExhaustion::classify(
                    request_usage,
                    self.provider.declaration().max_context_tokens,
                    self.provider.declaration().max_output_tokens,
                ) {
                    let detail = exhaustion.detail();
                    if self
                        .try_context_exhaustion_retry(
                            &mut overflow_retried,
                            events,
                            cancel,
                            exhaustion,
                    ...
                    return self.stop_with_detail(
                        ...
                        StopReason::ProviderError,
                        Some(format!(
                            "{detail}; compaction could not free enough room — start a new session or split the task"
                        )),
                    );
                }

                if let Some(message) = output_limit_message {
                    let _ = events.send(RuntimeEvent::Warning(message));
                }
...
fn retain_latest_images(mut messages: Vec<Message>, limit: usize) -> Vec<Message> {
    let mut to_remove = image_count(&messages).saturating_sub(limit);
    if to_remove == 0 {
        return messages;
    }
...
mod tests {
    ...
    async fn a_profile_swap_through_the_shared_handle_governs_the_next_dispatch() {
        ...
        let cancel = CancellationToken::new();
        let denied = runtime
            .run_user_shell_command_detailed("definitely-not-a-real-command", &cancel, true)
            .await
            .result;
        assert!(
    ...
    fn runtime_with_provider_and_store(
        provider: Arc<dyn ModelProvider>,
        store: Store,
        workspace: &Path,
        incognito: bool,
    ...
    fn runtime_with_provider(
        ...
        let dir = tempfile::tempdir().unwrap();
        let runtime =
            runtime_with_provider_and_store(provider, Store::open(dir.path()), dir.path(), false);
        (runtime, dir)
    }
    ...
    #[test]
    fn normal_session_persists_provider_metadata_and_reuses_it_on_the_next_run() {
        let dir = tempfile::tempdir().unwrap();
        let provider = fake_with_id("cached-provider");
        let declaration = provider.declaration().clone();

        let first = runtime_with_provider_and_store(
            provider.clone(),
            Store::open(dir.path()),
            dir.path(),
            false,
        ...
        let stored = reopened
            .get_provider_metadata("cached-provider")
            .unwrap()
            .unwrap();
        assert_eq!(stored["format_version"], PROVIDER_METADATA_FORMAT_VERSION);
        assert_eq!(stored["declaration"]["id"], "cached-provider");
        assert_eq!(
            sync_provider_metadata(&reopened, &declaration).unwrap(),
            ProviderMetadataSync::Current,
            "the next run reads and reuses an unchanged provider snapshot"
        );

        let second = runtime_with_provider_and_store(provider, reopened, dir.path(), false);
        drop(second);

        let round_trip: ProviderDeclaration = serde_json::from_value(stored["declaration"].clone())
            .expect("the persisted provider declaration remains typed and readable");
        assert_eq!(round_trip, declaration);
    }

    #[test]
    fn incognito_session_keeps_provider_metadata_in_memory_and_touches_no_disk() {
        let workspace = tempfile::tempdir().unwrap();
        let store = Store::ephemeral();
        let store_view = store.clone();
        let synthetic_root = store.root().to_path_buf();

        let runtime = runtime_with_provider_and_store(
            fake_with_id("incognito-provider"),
            store,
            workspace.path(),
            true,
        );
        ...
        assert!(store_view
            .get_provider_metadata("incognito-provider")
            .unwrap()
            .is_some());
        assert!(!synthetic_root.exists());
        assert!(!workspace.path().join(".localpilot").exists());
        drop(runtime);
    }

    #[test]
    fn a_damaged_provider_metadata_cache_is_replaced_by_the_live_declaration() {
        let dir = tempfile::tempdir().unwrap();
        let providers = dir.path().join(".localpilot").join("providers");
        std::fs::create_dir_all(&providers).unwrap();
        std::fs::write(providers.join("repair.json"), b"not-json").unwrap();

        let provider = fake_with_id("repair");
        let declaration = provider.declaration().clone();
        let runtime =
            runtime_with_provider_and_store(provider, Store::open(dir.path()), dir.path(), false);
        drop(runtime);

        let repaired = Store::open(dir.path())
            .get_provider_metadata("repair")
            .unwrap()
            .unwrap();
        let round_trip: ProviderDeclaration =
            serde_json::from_value(repaired["declaration"].clone()).unwrap();
        assert_eq!(round_trip, declaration);
    }
    ...
    fn switching_provider_retargets_and_resolves_the_new_default_model() {
        ...
        assert_eq!(
            runtime.store().get_provider_metadata("b").unwrap().unwrap()["declaration"]["id"],
            "b"
        );
    ...
    #[test]
    fn prompt_calibration_corrects_undercounts_without_trusting_overcounts() {
        let mut calibration = PromptTokenCalibration::default();
        // Even before a sample, the raw compaction budget keeps its 5% cushion.
        assert_eq!(calibration.raw_budget(10_000), 9_523);

        calibration.observe(1_000, 1_150);
        assert_eq!(calibration.calibrated_estimate(2_000), 2_300);
        assert_eq!(calibration.raw_budget(10_000), 8_281);

        // A prose-heavy sample that tokenizes below bytes/4 cannot make a later
        // code-heavy request less conservative than the original heuristic.
        calibration.observe(1_000, 800);
        assert_eq!(calibration.calibrated_estimate(2_000), 2_000);
    }

    #[test]
    fn context_exhaustion_requires_a_full_window_and_clear_output_headroom() {
        let exhausted = ContextExhaustion::classify(
            TokenUsage {
                input_tokens: 9_800,
                output_tokens: 200,
                ..Default::default()
        ...
        let true_output_cap = ContextExhaustion::classify(
            TokenUsage {
                input_tokens: 8_000,
                output_tokens: 2_000,
                ..Default::default()
        ...
        assert!(true_output_cap.is_none());

        let below_window = ContextExhaustion::classify(
            TokenUsage {
                input_tokens: 9_000,
                output_tokens: 200,
                ..Default::default()
