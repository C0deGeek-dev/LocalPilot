...
mod tests {
    ...
    fn no_stale_uncommitted_blocks_at_session_start() {
        ...
        assert!(outcomes
            .iter()
            .any(|(n, v)| *n == "no_stale_uncommitted" && matches!(v, RuleVerdict::Block(_))));
    }
    ...
    fn config_cannot_downgrade_a_critical_rule_to_allow() {
        ...
        assert!(outcomes
            .iter()
            .any(|(n, v)| *n == "suite_green" && matches!(v, RuleVerdict::Block(_))));
    }
    ...
    fn quality_gate_blocks_failed_block_denied_and_errored() {
        for outcome in [
            ...
            assert!(matches!(
                gate_verdict(&[outcome], RuleSeverity::Block),
                RuleVerdict::Block(_)
            ));
        }
    }
    ...
    fn quality_gate_takes_the_most_severe_outcome() {
        ...
        assert!(matches!(verdict, RuleVerdict::Block(_)));
    }
    ...
    fn quality_gate_is_critical_and_cannot_be_disabled() {
        ...
        assert!(outcomes
            .iter()
            .any(|(n, v)| *n == "quality_gate" && matches!(v, RuleVerdict::Block(_))));
    }
    ...
    fn check_before_launch_defaults_to_warn_and_respects_overrides() {
        ...
        assert!(blocking
            .evaluate(Trigger::PreShell, &ctx)
            .iter()
            .any(|(n, v)| *n == "check_before_launch" && matches!(v, RuleVerdict::Block(_))));
