//! Session persistence for Unshackled.
//!
//! The store owns the project-local `.unshackled/` directory: transcripts (one
...
use unshackled_core::{Message, SessionId};
...
/// A handle to a workspace's `.unshackled/` state directory.
#[derive(Debug, Clone)]
pub struct Store {
    root: PathBuf,
}
...
impl Store {
    /// Open the store under `workspace_root/.unshackled`.
    #[must_use]
    pub fn open(workspace_root: &Path) -> Self {
        Self {
            root: workspace_root.join(".unshackled"),
        }
    }

    /// Open the store at an explicit `.unshackled` directory.
    #[must_use]
    pub fn at(unshackled_dir: PathBuf) -> Self {
        Self {
            root: unshackled_dir,
        }
    }
...
mod tests {
    use super::*;
    use unshackled_core::{ContentBlock, Role};
