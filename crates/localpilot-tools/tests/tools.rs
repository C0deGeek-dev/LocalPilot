...
use localpilot_tools::{
    Confirmation, Reversibility, Tool, ToolContext, ToolContract, ToolError, ToolOutput,
    ToolRegistry,
};
use serde_json::json;
...
fn registry_with_tagged(reversibility: Reversibility, confirmation: Confirmation) -> ToolRegistry {
...
async fn irreversible_tool_prompts_under_relaxed_where_reversible_auto_approves() {
    ...
    let registry = registry_with_tagged(Reversibility::Irreversible, Confirmation::None);
    let denied = dispatch(
        &registry,
        "tagged",
        json!({}),
    ...
    let registry = registry_with_tagged(Reversibility::Reversible, Confirmation::None);
    let allowed = dispatch(
        &registry,
        "tagged",
        json!({}),
...
#[tokio::test]
async fn always_confirmation_prompts_under_relaxed() {
    let (_dir, ws) = workspace_with(&[]);
    let context = ctx(&ws, Interactivity::Interactive, true);
    let relaxed = PermissionEngine::new(Profile::Relaxed, Vec::new());
    let registry = registry_with_tagged(Reversibility::Reversible, Confirmation::Always);
    ...
    assert!(
        denied.is_error(),
        "Confirmation::Always must raise a relaxed auto-allow to a prompt"
    );
    assert_eq!(
        registry.get("git_commit").unwrap().contract().confirmation,
        Confirmation::Always,
        "git_commit is the consequential reversible builtin that uses the opt-in"
    );
}

#[tokio::test]
async fn bypass_and_unrestricted_scope_are_unchanged_by_confirmation() {
    let (_dir, ws) = workspace_with(&[]);
    let context = ctx(&ws, Interactivity::Interactive, true);

    for profile in [Profile::Bypass, Profile::Unrestricted] {
        // These profiles explicitly promise no prompts. The denying approver
        // must therefore remain untouched even for an Always-confirm tool.
        let registry = registry_with_tagged(Reversibility::Reversible, Confirmation::Always);
        let result = dispatch(
            &registry,
            "tagged",
        ...
        assert!(
            !result.is_error(),
            "{profile:?} must not prompt for Confirmation::Always"
        );
    }
}
