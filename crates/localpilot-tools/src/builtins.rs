...
use crate::contract::{
    Confirmation, ContentExpectation, Idempotency, PathEffectKind, Postcondition, Precondition,
...
/// `edit_file` can only operate on an existing target. Keep the optional
/// prior-read discipline alongside the always-on state requirement.
const EXISTING_PRIOR_READ_PATH: &[Precondition] = &[
    Precondition::State(StatePredicate::FileExists { path_arg: "path" }),
    Precondition::RequiresPriorRead { path_arg: "path" },
];
const PATH_EXISTS_AND_NON_EMPTY: &[Postcondition] = &[
    Postcondition::PathEffect {
        path_arg: "path",
...
const PATH_MODIFIED_AND_NON_EMPTY: &[Postcondition] = &[
    Postcondition::PathEffect {
        path_arg: "path",
...
impl Tool for WriteFile {
    ...
    fn contract(&self) -> ToolContract {
        ToolContract {
            ...
            postconditions: PATH_EXISTS_AND_NON_EMPTY,
...
impl Tool for EditFile {
    ...
    fn contract(&self) -> ToolContract {
        ToolContract {
            ...
            reversibility: Reversibility::ReversibleWithArtifact,
            idempotency: Idempotency::NonIdempotent,
            preconditions: EXISTING_PRIOR_READ_PATH,
            postconditions: PATH_MODIFIED_AND_NON_EMPTY,
            verification: VerificationMethod::ReadBack { tool: "read_file" },
            ..ToolContract::default()
        }
    }
...
impl Tool for GitCommit {
    ...
    fn contract(&self) -> ToolContract {
        ToolContract {
            ...
            reversibility: Reversibility::ReversibleWithArtifact,
            idempotency: Idempotency::NonIdempotent,
            // Creating durable history is consequential even though the commit
            // can be reverted or reset with the repository as its artifact.
