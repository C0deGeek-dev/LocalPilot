...
/// How many session-span hits are pulled as pack candidates for one task.
/// Bounded because spans are numerous and cheap to produce; the pack's own
/// selection then decides how many survive.
const SPAN_CANDIDATES: usize = 24;
...
pub enum IngestError {
    ...
    Disabled,
    #[error(
        "ingest model-call budget reached after {used} call(s) (max_model_calls = {limit}); increase [ingest] max_model_calls or set it to 0 for unlimited"
    )]
    ModelCallBudgetExceeded { limit: u64, used: u64 },
...
impl ClassifiedFile {
    fn without_chunks(entry: ManifestEntry) -> Self {
        Self {
            entry,
            chunk_count: 0,
        }
...
impl ModelCallBudget {
    fn new(max_model_calls: u64) -> Self {
        Self {
            limit: NonZeroU64::new(max_model_calls),
            used: 0,
        }
    }
    ...
    fn record_call(&mut self) -> Result<(), IngestError> {
        if let Some(limit) = self.limit {
            if self.used >= limit.get() {
                return Err(IngestError::ModelCallBudgetExceeded {
                    limit: limit.get(),
                    used: self.used,
                });
            }
...
pub fn preview(project_root: &Path, config: &IngestConfig) -> Result<PreviewManifest, IngestError> {
    ...
    let mut estimated_candidate_files = 0_u64;
    let mut estimated_skipped_files = 0_u64;
    let mut estimated_candidate_bytes = 0_u64;
    let mut estimated_tokens = 0_u64;
    let mut estimated_model_calls = 0_u64;
    let model_calls_active =
        config.embed_chunks && resolve_embed_capability(&root).embeddings().is_some();
    ...
    for result in walker(&root) {
        ...
        let path = entry.path();
        let ClassifiedFile {
            entry: mut manifest_entry,
            chunk_count,
        } = classify(&root, path, config, &localmind_excludes)?;
        ...
        match manifest_entry.status {
            CandidateStatus::Candidate => {
                estimated_candidate_files = estimated_candidate_files.saturating_add(1);
                estimated_candidate_bytes =
                    estimated_candidate_bytes.saturating_add(manifest_entry.size_bytes);
                estimated_tokens = estimated_tokens.saturating_add(manifest_entry.token_estimate);
                if model_calls_active {
                    estimated_model_calls = estimated_model_calls.saturating_add(chunk_count);
                }
            }
            _ => estimated_skipped_files = estimated_skipped_files.saturating_add(1),
        }
    ...
    if model_calls_active {
        ...
        estimated_model_calls = estimated_model_calls.saturating_add(
            u64::try_from(project_context_chunks(&root, &prefix_policy).len()).unwrap_or(u64::MAX),
        );
    }
    // Keep this construction exhaustive: a new serialized estimate field must
    // be assigned deliberately here instead of silently inheriting a default.
    let estimates = BudgetEstimate {
        candidate_files: estimated_candidate_files,
        skipped_files: estimated_skipped_files,
        candidate_bytes: estimated_candidate_bytes,
        token_estimate: estimated_tokens,
        model_calls: estimated_model_calls,
        over_file_budget: estimated_candidate_files > config.max_files,
        over_byte_budget: estimated_candidate_bytes > config.max_run_bytes,
        over_token_budget: estimated_tokens > config.max_tokens,
    };
    Ok(PreviewManifest {
...
pub fn run_with_progress(
    ...
    let mut model_call_budget = ModelCallBudget::new(config.max_model_calls);
        ...
        .filter(|entry| entry.status == CandidateStatus::Candidate)
    {
        ...
        match chunk_file(&absolute, &entry.path, hash, &prefix_policy) {
            Ok(file_chunks) => {
                ...
                if let Err(error) = embed_chunks_best_effort(
                    &store,
                    embed_endpoint,
                    &file_chunks,
                    &mut model_call_budget,
                ) {
                    stop_for_model_call_budget(&store, &ingest_dir, &mut job, &entry.path, &error)?;
                    return Err(error);
                }
                job.completed_files = job.completed_files.saturating_add(1);
            }
    ...
    let doc_files_indexed = if config.docs_index && job.status == JobStatus::Running {
        // The LocalMind documentation bridge performs its own embedding calls.
        // With a finite LocalPilot call budget, keep the hard bound by indexing
        // the text without vectors; an uncapped run preserves the normal
        // best-effort semantic-doc behavior.
        let embed_docs = config.embed_chunks && model_call_budget.is_unlimited();
        bridge_docs_index(&root, &ingest_dir, &manifest, embed_docs)
    } else {
        0
    };
...
pub fn compute_pack(
    ...
    // Session spans reach *every* session, which is what the recent-session
    // source below cannot do — it reads the newest summary's key points. A span
    // is transcript rather than knowledge, so it carries a low quality weight
    // and no reserve: it competes in the shared pool and outranks nothing by
    // default.
    //
    // Best-effort throughout. A project with no span index is the normal case,
    // and an unreadable one must cost this query its spans, never the query.
    //
    // The existence check comes first because opening the store *creates* it.
    // This is a read path reached by an ordinary prompt, and a plain prompt must
    // never create project files — a rule this would otherwise have broken by
    // leaving an empty database behind on every search.
    if crate::span_store::has_span_index(&root) {
        if let Ok(store) = crate::span_store::SpanStore::open(&root) {
            if let Ok(hits) = store.search(task, SPAN_CANDIDATES) {
                let total = hits.len().max(1);
                // Resolved in one pass: the index holds no text, and doing this per
                // hit would re-chunk a transcript once per result from it.
                let texts = store.span_texts(&hits);
                for (index, hit) in hits.into_iter().enumerate() {
                    // Never serve the live conversation back to itself as knowledge.
                    if exclude_session.is_some_and(|active| active == hit.session_id) {
                        continue;
                    }
                    let Some(text) = texts.get(index).cloned().flatten() else {
                        continue;
                    };
                    // bm25 order is the only relevance signal the index gives, so
                    // rank position becomes relevance: first hit ~1.0, last ~0.
                    #[allow(clippy::cast_precision_loss)]
                    let relevance = 1.0 - (index as f32 / total as f32);
                    candidates.push(PackCandidate {
                        source: PackSource::SessionSpan,
                        id: crate::span_store::span_locator(&hit),
                        path: Some(format!(
                            "{}:{}-{}",
                            hit.session_id, hit.start_line, hit.end_line
                        )),
                        score: 6,
                        relevance,
                        token_estimate: (text.chars().count() as u64 / 4).max(1),
                        snippet: text,
                        stale: false,
                        recency: 10,
                        file_match: false,
...
fn classify(
...
) -> Result<ClassifiedFile, IngestError> {
    ...
    if explicit_exclude {
        ...
        return Ok(ClassifiedFile::without_chunks(entry));
    }
...
fn chunk_count(text: &str) -> u64 {
    let mut chunks = 0_u64;
    let mut current_bytes = 0_usize;
    for line in text.lines() {
        let line_bytes = line.len().saturating_add(1);
        if current_bytes > 0 && current_bytes.saturating_add(line_bytes) > CHUNK_BYTES {
            chunks = chunks.saturating_add(1);
            current_bytes = 0;
        }
        current_bytes = current_bytes.saturating_add(line_bytes);
    }
    if current_bytes > 0 || text.is_empty() {
        chunks = chunks.saturating_add(1);
    }
    chunks
}
...
fn embed_chunks_best_effort(
    ...
    for chunk in chunks {
        ...
        // Skip a chunk whose vector is current in both respects. Matching on the
        // fingerprint alone left a model change unrepairable: the text is
        // unchanged, so the skip fires and the chunk keeps a vector from a model
        // the search side no longer uses.
        match store.vector_identity(&chunk.id) {
            Ok(Some((existing, model))) if existing == fingerprint && model == endpoint.model() => {
                continue
            }
            Ok(_) => {}
            Err(error) => {
...
fn stop_for_model_call_budget(
    ...
    // The current file/context may have been stored before its embedding pass.
    // Remove that incomplete unit so a refresh can retry it instead of treating
    // the partially embedded path as fresh.
    store.forget(incomplete_path)?;
    job.status = JobStatus::Paused;
...
pub(crate) fn fnv_hash_hex(bytes: &[u8]) -> String {
...
mod tests {
    ...
    fn three_chunk_text() -> String {
        let line = "x".repeat(CHUNK_BYTES / 2);
        format!("{line}\n{line}\n{line}\n")
    }
    ...
    #[test]
    fn chunk_count_matches_the_ingest_splitter() {
        ...
        for text in fixtures {
            let chunks = chunk_text(&text, "fixture.txt", "hash", &policy);
            assert_eq!(
                chunk_count(&text),
    ...
    #[test]
    fn preview_reports_each_configured_chunk_embedding_call() {
        let dir = tempfile::tempdir().unwrap();
        fs::write(dir.path().join("corpus.txt"), three_chunk_text()).unwrap();
        write_inference_config(dir.path(), "http://127.0.0.1:1");
        let cfg = IngestConfig {
            exclude: vec![".localmind.toml".to_string()],
            docs_index: false,
            ..config()
        };
        ...
        assert_eq!(manifest.estimates.model_calls, 3);
    }

    #[test]
    fn model_call_budget_stops_before_the_third_call_with_a_clear_error() {
        ...
        let error = run(dir.path(), &cfg, RunMode::Full).unwrap_err();

        assert!(matches!(
            error,
            IngestError::ModelCallBudgetExceeded { limit: 2, used: 2 }
        ));
        assert_eq!(count.load(Ordering::SeqCst), 2);
        let job = status(dir.path()).unwrap().unwrap();
        assert_eq!(job.status, JobStatus::Paused);
        assert!(job
            .message
            .as_deref()
            .is_some_and(|message| message.contains("set it to 0 for unlimited")));
        assert!(
            stored_chunks(dir.path()).is_empty(),
            "the partly embedded file must be retryable on resume"
        );
    }

    #[test]
    fn zero_model_call_budget_is_explicitly_unlimited() {
        use std::sync::atomic::Ordering;
        ...
        let summary = run(dir.path(), &cfg, RunMode::Full).unwrap();
    ...
    #[test]
    fn capped_run_indexes_markdown_without_unbudgeted_doc_embedding_calls() {
        use std::sync::atomic::Ordering;
        ...
        assert_eq!(summary.job.status, JobStatus::Completed);
        assert_eq!(summary.doc_files_indexed, 1);
        assert_eq!(count.load(Ordering::SeqCst), 1);
        let persistence = localmind_store::MemoryPersistence::open_project(dir.path()).unwrap();
        assert!(persistence.doc_chunk_count().unwrap() >= 1);
    }
    ...
    fn unchanged_chunks_are_not_re_embedded() {
        ...
        let mut budget = ModelCallBudget::new(0);
        embed_chunks_best_effort(&store, Some(&endpoint), &chunks, &mut budget).unwrap();
