...
pub use codegraph::{
    ...
    codegraph_reindex_all, ArchitectureSummary, CodeGraphSummary, ExportFormat,
    GraphLanguageSummary, GraphPackageSummary, GraphSymbolSummary, SymbolReport,
};
...
pub use ops::{
    ...
    context_for, context_hits, context_hits_gated_for_eval, flag_unhelpful_lesson, freshness_pass,
    lessons_flagged_for_review, memory_delete, memory_disable_injection, memory_enable_injection,
    memory_injection_enabled, memory_lifecycle, memory_list, memory_list_readonly,
    pre_fix_relevance_cosines_for_eval, promote, record_memory_usage, revalidate,
    revalidation_proposal, review_decide, review_list, review_list_readonly, review_purge,
    review_show, search, search_readonly, skill_activate, skill_body, skill_show, skills_active,
    skills_generate, skills_list, ActiveSkillInfo, AuditEntry, FreshnessFlagOut, FreshnessOutcome,
    FreshnessParams, MemoryLifecycle, MemorySummary, RevalidationOutcome, ReviewSummary,
    ReviewVerdict, SearchHit, SkillDraftInfo, StaleClearOutcome,
};
...
pub use span_eval::{
    query_set, score_query, summarise, synthetic_corpus, synthetic_locator, ClassSummary,
    DenseFixture, EvalQuery, JudgmentSet, QualityReport, QueryClass, QueryOutcome,
};
pub use span_store::{
    has_span_index, parse_span_locator, span_locator, FetchedSpan, IndexReport, SpanHit, SpanMiss,
    SpanStore, SpanStoreError,
};
pub use tool_use::{tool_use_candidate, ToolUseSignal};
pub use transcript::{
    detect_schema, read_transcript, RecoveryReport, Span, SpanKind, TranscriptRead,
