...
/// Resolve session-span locators to bodies, explaining every one that cannot be.
///
/// Spans resolve through the transcript they point into, because the span index
/// stores no text. Anything unresolvable is pushed onto `rejected` with the
/// reason — an id that silently returns nothing looks identical to an id that
/// returned an empty span, and the model cannot tell those apart.
pub fn fetch_spans(
    project_root: &Path,
    ids: &[String],
    rejected: &mut Vec<(String, &'static str)>,
) -> Vec<FetchedBody> {
    if ids.is_empty() {
        return Vec::new();
    }
    let Ok(store) = crate::span_store::SpanStore::open(project_root) else {
        for id in ids {
            rejected.push((id.clone(), "the session-span index could not be opened"));
        }
        return Vec::new();
    };
    let mut bodies = Vec::with_capacity(ids.len());
    for id in ids {
        match store.fetch_span(id) {
            Ok(Ok(span)) => bodies.push(FetchedBody {
                token_cost: estimate_tokens(&span.text),
                id: span.locator,
                path: format!("session {}", span.session_id),
                start_line: span.start_line as u64,
                end_line: span.end_line as u64,
                body: span.text,
            }),
            Ok(Err(miss)) => rejected.push((id.clone(), span_miss_reason(miss))),
            Err(_) => rejected.push((id.clone(), "the session-span index is unreadable")),
        }
    }
    bodies
}

fn span_miss_reason(miss: crate::span_store::SpanMiss) -> &'static str {
    match miss {
        crate::span_store::SpanMiss::NotASpan => "not a session-span locator",
        crate::span_store::SpanMiss::StaleContract => {
            "issued under an older chunking contract, so its position no longer identifies the              same text — search again to get a current locator"
        }
        crate::span_store::SpanMiss::Gone => {
            "its session or that position in it no longer exists"
        }
        crate::span_store::SpanMiss::Changed => {
            "its transcript changed after indexing, so that position now holds different text —              search again rather than trusting this locator"
        }
    }
}
