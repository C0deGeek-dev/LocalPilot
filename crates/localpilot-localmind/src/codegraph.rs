...
fn prepare_codegraph_reindex(
    ...
    (
        ...
        GraphStore,
        Reindexer,
        localmind_codegraph::ReindexPlan,
    ),
    LearningError,
> {
    ...
    let store = GraphStore::open_project(project_root)
        .map_err(|error| LearningError::Graph(error.to_string()))?;
    let reindexer = Reindexer::new().map_err(|error| LearningError::Graph(error.to_string()))?;
    let plan = reindexer
        .plan(&boundary, &candidates, &store)
        .map_err(|error| LearningError::Graph(error.to_string()))?;
    Ok((boundary, store, reindexer, plan))
}
...
pub fn codegraph_reindex(
    ...
    let (boundary, store, mut reindexer, mut plan) = prepare_codegraph_reindex(project_root)?;
...
/// Reindex the whole project through bounded, resumable batches. The work plan
/// is built once, cumulative progress is reported before work and after every
/// committed batch, and a cancellation request stops before the next batch.
/// A cancelled run returns normally with `remaining > 0`, so callers can report
/// committed progress honestly and a later run can resume from stored hashes.
///
/// `batch_limit` controls cancellation latency, not total work; zero is treated
/// as one to guarantee forward progress.
///
/// # Errors
/// Returns [`LearningError`] when configuration, graph planning, parsing, or
/// persistence fails.
pub fn codegraph_reindex_all(
    project_root: &Path,
    batch_limit: usize,
    is_cancelled: &dyn Fn() -> bool,
    on_progress: &mut dyn FnMut(&CodeGraphSummary),
) -> Result<CodeGraphSummary, LearningError> {
    if is_cancelled() {
        return Ok(CodeGraphSummary::default());
    }
    let (boundary, store, mut reindexer, mut plan) = prepare_codegraph_reindex(project_root)?;
    ...
    while !plan.is_complete() && !is_cancelled() {
        let report = reindexer
            .run(&boundary, &store, &mut plan, batch_limit)
            .map_err(|error| LearningError::Graph(error.to_string()))?;
        summary.reindexed += report.reindexed;
        summary.pruned += report.pruned;
        summary.rejected = plan.rejected.len();
        summary.remaining = plan.remaining();
        on_progress(&summary);
    }
...
mod tests {
    use super::{
        codegraph_export, codegraph_inspect, codegraph_overview, codegraph_reindex,
        codegraph_reindex_all, ExportFormat,
    };
    use std::cell::Cell;
    use std::fs;
    ...
    #[test]
    fn full_reindex_reports_remaining_and_resumes_after_cancellation(
        ...
        let partial = codegraph_reindex_all(root, 1, &|| cancel.get(), &mut |summary| {
            remaining.push(summary.remaining);
            if summary.reindexed == 1 {
                cancel.set(true);
            }
        })?;
        assert_eq!(partial.reindexed, 1);
        assert_eq!(partial.remaining, 2);
        assert_eq!(remaining, vec![3, 2]);
        assert_eq!(codegraph_overview(root)?.files, 1);

        let completed = codegraph_reindex_all(root, 2, &|| false, &mut |_| {})?;
        assert_eq!(completed.reindexed, 2);
        assert_eq!(completed.unchanged, 1);
        assert_eq!(completed.remaining, 0);
        assert_eq!(codegraph_overview(root)?.files, 3);
        Ok(())
    }

    #[test]
    fn full_reindex_cancelled_before_preflight_creates_nothing(
    ) -> Result<(), Box<dyn std::error::Error>> {
        let temp_dir = tempfile::tempdir()?;
        let root = temp_dir.path();
        let summary = codegraph_reindex_all(root, 1, &|| true, &mut |_| {})?;
