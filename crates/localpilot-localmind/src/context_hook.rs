...
impl InjectionPolicy {
    ...
    ///
    /// **A hit containing the query's subject is exempt from the relevance
    /// floor.** The floor is a second opinion for a hit that matched only the
    /// shape of the question; a hit that matched what the question is *about* has
    /// already answered it lexically, and a floor high enough to drop off-topic
    /// noise is also high enough to drop real answers phrased at a distance. On a
    /// real store the same memory is the correct answer to one prompt at cosine
    /// 0.353 and irrelevant noise for another at 0.351 — the exemption is what
    /// tells those two apart, and cosine alone cannot.
    fn skips(
        &self,
        score: i64,
        category: &str,
        ...
        score < self.min_score
            ...
            || (self.min_cosine > 0.0
                && !subject_matched
                && cosine.is_some_and(|c| c < self.min_cosine))
    }
}
        ...
        match crate::ops::context_hits(&self.root, prompt, task_language) {
            Ok(hits) => {
                ...
                for hit in hits {
                    ...
                    if policy.skips(hit.score, &hit.category, hit.cosine, hit.subject_matched) {
                        continue;
                    }
...
mod tests {
    ...
    fn the_cosine_gate_skips_an_off_topic_hit_but_passes_an_on_topic_one() {
        ...
        assert!(!disabled.skips(10, "CodePattern", Some(0.01), false));
    }

    #[test]
    fn a_hit_containing_the_query_subject_is_exempt_from_the_relevance_floor() {
        let gate = InjectionPolicy {
            min_cosine: 0.36,
            ..InjectionPolicy::fixed_default()
        };
        // The same cosine, decided two ways. This is not a hypothetical: on a real
        // store one memory is the correct answer to one prompt at 0.353 and
        // irrelevant noise for another at 0.351, and cosine alone cannot tell them
        // apart. What separates them is whether the body contains what the prompt
        // was asking about.
        assert!(
            gate.skips(10, "CodePattern", Some(0.30), false),
            "a hit that matched only the shape of the question must face the floor"
        );
        assert!(
            !gate.skips(10, "CodePattern", Some(0.30), true),
            "a hit containing the query's subject has already answered it lexically"
        );
    }
