...
fn source_label(source: PackSource) -> &'static str {
    match source {
        ...
        // Named for what it is. A span is conversation, not a fact the project
        // asserts, and the label is the only thing standing between the two
        // when a model reads a result list.
...
impl Tool for KnowledgeSearch {
    ...
    async fn invoke(&self, input: Value, ctx: &ToolContext<'_>) -> Result<ToolOutput, ToolError> {
        ...
        // A missing *ingest* index no longer means there is nothing to search:
        // session spans are indexed separately and reach every past session. Bail
        // out only when neither index exists, or a project with sessions but no
        // ingested files would be told it has no knowledge while holding plenty.
        ...
        for entry in visible.iter().take(limit) {
            ...
            let fetch = if matches!(entry.source, PackSource::Ingest | PackSource::SessionSpan) {
                "fetchable"
            } else {
                "not fetchable"
            };
