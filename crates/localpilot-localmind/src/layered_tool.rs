...
fn non_fetchable_reason(id: &str) -> Option<&'static str> {
    ...
    } else if id.starts_with("span:") {
        // Fetchable, and by a different route: a span resolves through the
        // transcript it points into, because the span index stores no text.
        None
    } else {
        None
    }
...
impl Tool for KnowledgeFetch {
    ...
    async fn invoke(&self, input: Value, ctx: &ToolContext<'_>) -> Result<ToolOutput, ToolError> {
        ...
        let (fetchable, rejected) = partition_fetchable(ids);
        let root = ctx.workspace.root();
        let (span_ids, chunk_ids): (Vec<String>, Vec<String>) = fetchable
            .into_iter()
            .partition(|id| id.starts_with("span:"));
        let mut bodies = crate::layered::fetch_layer(root, &chunk_ids)
            .map_err(|e| ToolError::Failed(e.to_string()))?;
        let mut rejected = rejected;
        bodies.extend(crate::layered::fetch_spans(root, &span_ids, &mut rejected));
