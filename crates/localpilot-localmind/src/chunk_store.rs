...
impl ChunkStore {
    ...
    /// The stored embedding fingerprint **and model** for a chunk, or `None` when
    /// the chunk has no vector yet — the re-embed signal.
    ///
    /// A chunk is skipped only when both match what is about to be embedded.
    /// Comparing the fingerprint alone made a model change unrepairable: the text
    /// had not changed, so the skip fired and the chunk kept a vector from a model
    /// the search side no longer uses.
    pub(crate) fn vector_identity(
        &self,
        chunk_id: &str,
    ) -> Result<Option<(String, String)>, IngestError> {
        self.connection
            .query_row(
                "SELECT source_fingerprint, model FROM ingest_chunk_vectors WHERE chunk_id = ?1",
                params![chunk_id],
