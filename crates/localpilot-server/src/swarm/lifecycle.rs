...
mod tests {
    ...
    #[tokio::test]
    async fn a_legacy_failed_snapshot_loads_as_departed_from_primary_or_backup() {
        for use_backup in [false, true] {
            let dir = tempfile::tempdir().unwrap();
            let store = SnapshotStore::new(dir.path());
            let swarm = SwarmId::new("legacy-failure");
            let lead = SessionId::new();
            let mut member = SwarmMember::worker(SessionId::new(), "alpha", lead);
            member.status = MemberStatus::Departed;
            let expected = SwarmSnapshot {
                swarm: swarm.clone(),
                revision: 3,
                members: vec![member],
                coordinator: Some(lead),
                plan: Some(TaskPlan::new(
                    "recover the work",
                    PlanMode::Light,
            ...
            legacy["members"][0]["status"] = serde_json::json!({
                "state": "failed",
                "reason": "the worker stopped unexpectedly"
            });
            let path = if use_backup {
                std::fs::write(store.primary(&swarm), b"{ torn").unwrap();
                store.backup(&swarm)
            } else {
                store.primary(&swarm)
            };
            ...
            let mut recovered = store.load(&swarm).await.unwrap().unwrap();
            assert_eq!(recovered, expected);
            assert!(!recovered.members[0].status.is_active());

            // New writes use only the live status vocabulary.
            recovered.revision += 1;
            assert!(store.save(&recovered).await.unwrap());
            let saved: serde_json::Value =
                serde_json::from_slice(&std::fs::read(store.primary(&swarm)).unwrap()).unwrap();
            assert_eq!(
                saved["members"][0]["status"],
                serde_json::json!({"state": "departed"})
            );
            assert_eq!(store.load(&swarm).await.unwrap(), Some(recovered));
        }
    }
