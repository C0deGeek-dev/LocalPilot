...
pub enum MemberStatus {
    ...
    /// Historic snapshots may call this `failed`; accept that spelling on read
    /// and ignore its obsolete `reason` field, but only write `departed`.
    #[serde(alias = "failed")]
    Departed,
}
...
mod tests {
    ...
    #[test]
    fn live_member_statuses_keep_their_wire_format() {
        for (status, state) in [
            (MemberStatus::Active, "active"),
            (MemberStatus::Finished, "finished"),
            (MemberStatus::Departed, "departed"),
        ] {
            let json = serde_json::json!({"state": state});
            assert_eq!(serde_json::to_value(&status).unwrap(), json);
            assert_eq!(
                serde_json::from_value::<MemberStatus>(json).unwrap(),
                status
            );
        }
    }
    ...
    #[test]
    fn unrelated_unknown_member_statuses_are_still_rejected() {
        assert!(
            serde_json::from_value::<MemberStatus>(serde_json::json!({"state": "actvie"})).is_err()
        );
    }
