...
impl AppModel {
    ...
    fn localmind_graph_is_active(&self) -> bool {
        matches!(self.active_body(), ActiveBody::LocalMind)
            && self
                .localmind_tab
                .as_ref()
                .filter(|state| state.kind == TakeoverKind::LocalMind)
                .and_then(|state| state.localmind.as_ref())
                .is_some_and(|localmind| localmind.section == LocalMindSection::Graph)
    }
    ...
    fn localmind_review_intent(
        ...
        let allowed = Self::review_allows(row, action);
    ...
    /// on it and by the footer that advertises it, so the two cannot drift.
    ...
    ///
    /// A refusal used to be indistinguishable from a dead key: the intent
    /// returned `None` and the caller mapped it to [`AppCommand::None`]. Every
    ...
    fn localmind_review_command(&mut self, action: LocalMindReviewAction) -> AppCommand {
        ...
        if let Some(reason) = self.localmind_review_refusal(action) {
            self.apply_runtime(RuntimeUpdate::Notice(reason));
        }
        AppCommand::None
    }

    /// Why `action` is unavailable for the selected row, in the operator's
    /// terms. `None` when the refusal already speaks for itself — an unset
    /// reviewer opens the identity editor, which is its own answer.
    fn localmind_review_refusal(&self, action: LocalMindReviewAction) -> Option<String> {
        let state = self
            .localmind_tab
            .as_ref()
            .filter(|state| state.kind == TakeoverKind::LocalMind)?;
        let localmind = state.localmind.as_ref()?;
        if localmind.section != LocalMindSection::Review || localmind.reviewer.trim().is_empty() {
            return None;
        }
        let Some(row) = localmind.data.review.get(state.selected) else {
            return Some("No review candidate is selected.".to_string());
        };
    ...
    /// The most recent notice on the active timeline.
    ///
    /// The LocalMind tab replaces the session body, so the work chrome and the
    /// notices that body normally carries are not drawn while it is open. The
    /// tab renders this instead, which is what makes an operation in flight and
    /// a refused action visible from the pane that produced them.
    #[must_use]
    pub fn last_notice(&self) -> Option<&str> {
        self.active_timeline()
            ...
            .iter()
            .rev()
            .find(|item| item.kind == ItemKind::Notice)
    ...
    fn handle_localmind_input(&mut self, action: InputAction) -> AppCommand {
        match action {
            ...
            InputAction::Insert(text)
                if text.eq_ignore_ascii_case("r") && self.localmind_graph_is_active() =>
            {
            ...
            {
                self.localmind_review_command(LocalMindReviewAction::Reject)
            }
            InputAction::Insert(text)
                if text.eq_ignore_ascii_case("p") && self.localmind_review_is_active() =>
            {
                self.localmind_review_command(LocalMindReviewAction::Promote)
            }
            InputAction::AcceptCompletion => {
...
mod tests {
    ...
    /// Drive a fresh model to the Review section with a reviewer identity set,
    /// so a review key exercises the row rule rather than the identity prompt.
    fn review_ready(row: LocalMindReviewRow) -> AppModel {
        let mut app = model();
        app.open_localmind(localmind_data(vec![row]));
        for _ in 0..3 {
            let _ = app.handle_input(InputAction::AcceptCompletion, 80);
        }
        assert_eq!(app.localmind_section(), Some(LocalMindSection::Review));
        let _ = app.handle_input(InputAction::Insert("i".to_string()), 80);
        for letter in ["d", "e", "v"] {
            let _ = app.handle_input(InputAction::Insert(letter.to_string()), 80);
        }
        let _ = app.handle_input(InputAction::Submit, 80);
        app
    }

    /// A refused review action used to be indistinguishable from a dead key.
    /// Every refusal now names the rule that blocked it, and the reason reaches
    /// the notice the LocalMind tab renders.
    #[test]
    fn a_refused_review_action_says_which_rule_blocked_it() {
        let mut app = review_ready(review_row("Accepted", false, false));
        ...
        assert!(reason.contains("Cannot accept"), "{reason}");
        assert!(reason.contains("pending"), "{reason}");

        // The verb the row *can* take still produces its command.
        assert!(matches!(
    ...
    #[test]
    fn graph_reindex_intent_is_typed_and_graph_only() {
        let mut app = model();
        app.open_localmind(localmind_data(Vec::new()));

        assert_eq!(
            app.handle_input(InputAction::Insert("r".to_string()), 80),
            AppCommand::None,
            "Docs must not emit the Graph write intent"
        );
        assert_eq!(
            app.handle_input(InputAction::AcceptCompletion, 80),
            AppCommand::None
        ...
        assert_eq!(
            app.handle_input(InputAction::Insert("r".to_string()), 80),
            AppCommand::None,
            "Memory must not emit the Graph write intent"
        );
    }
