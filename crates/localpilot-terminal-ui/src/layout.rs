...
impl FrameLayout {
    ...
    #[must_use]
    pub fn calculate_pair(area: Rect, requested_editor_rows: u16) -> Option<Self> {
        Self::calculate_pair_for_mode(area, requested_editor_rows, false)
    }
...
mod tests {
    ...
    fn pair_thresholds_preserve_a_label_and_three_timeline_rows() {
        assert!(FrameLayout::calculate_pair(Rect::default(), u16::MAX).is_none());
        assert!(FrameLayout::calculate_pair(Rect::new(0, 0, 29, 24), 1).is_none());
        assert!(FrameLayout::calculate_pair(Rect::new(0, 0, 61, 10), 1).is_none());

        let narrow =
            FrameLayout::calculate_pair(Rect::new(0, 0, 30, 11), 1).expect("minimum narrow pair");
        ...
        let last_active_only = FrameLayout::calculate_pair(Rect::new(0, 0, 60, 11), 1)
        ...
        let wide =
            FrameLayout::calculate_pair(Rect::new(0, 0, 61, 11), 1).expect("minimum wide pair");
    ...
    fn wide_pair_tiles_non_zero_origin_with_deterministic_odd_spare() {
        let layout = FrameLayout::calculate_pair(Rect::new(7, 5, 62, 20), 1).expect("pair");
    ...
    fn pair_editor_cap_reserves_the_label_at_the_first_competing_row() {
        ...
        let pair_at_pair_cap = FrameLayout::calculate_pair(area, 2).expect("pair at cap");
        ...
        let pair_over_pair_cap = FrameLayout::calculate_pair(area, 3).expect("pair over cap");
        assert_eq!(
