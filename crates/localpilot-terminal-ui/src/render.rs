    ...
    match app.active_body() {
        ...
        ActiveBody::LocalMind => {
            if let Some(localmind) = app.localmind_tab() {
                ...
                // This body replaces the session body, which is where the work
                // chrome and the notices normally render. Give the operation
                // status one row here, or an operation started from this tab
                // reports its progress, its result and its refusals to a
                // surface nothing draws while the tab is open.
                let status = localmind_status_line(app);
                let status_height = u16::from(status.is_some())
                    .min(area.height.saturating_sub(tabs_height).saturating_sub(1));
                ...
                if let (Some(status), 1) = (status, status_height) {
                    frame.render_widget(
                        Paragraph::new(truncate_end(&status, status_area.width.saturating_sub(2)))
                            .style(theme(app).ui(UiRole::Accent)),
                        Rect::new(
...
fn render_takeover(
    ...
    // The reviewer identity belongs to the review decision, and `i` only edits
    // it on Review. Showing the line on every section advertised a key that
    // does nothing on five of the six.
    if let Some(localmind) = takeover
        .localmind
    ...
    let footer = match takeover.kind {
        ...
        TakeoverKind::LocalMind => takeover.localmind.map_or_else(String::new, |localmind| {
            // An operation in flight refuses both the reindex and the review
            // actions, so the footer stops offering the keys that would be
            // refused rather than advertising them and going quiet.
                ...
                && localmind.editing_replacement
            {
                ...
                if busy {
                    ...
                    "Tab/Shift+Tab section · ↑/↓ scroll · r reindex · Ctrl+C copy · Esc close"
                        .to_string()
                }
            } else if localmind.section == LocalMindSection::Review {
...
/// The one-line status the LocalMind tab renders in place of the session
/// body's work chrome: the operation in flight with its elapsed time, the
/// latest notice, or both. `None` when there is nothing to say, so the tab
/// gives the row back to its content.
fn localmind_status_line(app: &AppModel) -> Option<String> {
    match (working_status(app), app.last_notice()) {
        (Some(working), Some(notice)) => Some(format!("{working} · {notice}")),
        (Some(working), None) => Some(working),
...
mod tests {
    ...
    fn localmind_renders_sections_read_data_and_review_controls() {
        ...
        terminal
            ...
            .expect("render graph");
        let rendered = terminal.backend().to_string();
        assert!(rendered.contains("12 files · 44 symbols"));
        assert!(rendered.contains("r reindex"));
        ...
        // The selected candidate is pending, so it can be accepted or rejected
        // and cannot be promoted. The footer advertises exactly the keys the
    ...
    /// from the tab used to report its progress, its result and its refusals
    /// to a surface nothing drew while the tab was open. The tab carries the
    /// status itself now.
    #[test]
    fn the_localmind_tab_shows_the_operation_it_started() {
        let mut app = model();
        app.open_localmind(localmind_fixture());
        let _ = app.handle_input(crate::InputAction::AcceptCompletion, 80);
        assert_eq!(
            app.localmind_section(),
            Some(crate::LocalMindSection::Graph)
        );
        ...
        terminal
            ...
            .expect("render idle graph");
        assert!(terminal.backend().to_string().contains("r reindex"));

        app.begin_work_with_label("Reindexing LocalMind graph");
        app.apply_runtime(crate::RuntimeUpdate::Notice(
            "code graph: 9 reindexed · 3 remaining".to_string(),
        ));
        terminal
            .draw(|frame| {
                let _ = render(frame, &app);
            })
            .expect("render working graph");
        let rendered = terminal.backend().to_string();
        assert!(
            rendered.contains("Reindexing LocalMind graph"),
            "the run must be visible from the tab that started it: {rendered}"
        );
        assert!(
            rendered.contains("9 reindexed"),
            "per-batch progress must reach the tab: {rendered}"
        );
        assert!(
            rendered.contains("reindex running"),
            "the footer must report the run instead of offering the key again"
        );
        assert!(
            !rendered.contains("r reindex"),
            "a second press is refused while the run is in flight, so it is not offered"
        );
    }

    /// `i` edits the reviewer identity on Review and nowhere else, so the line
    /// that offers it is drawn on Review and nowhere else. It used to sit under
    /// every section, advertising a key that did nothing on five of the six.
    #[test]
    fn the_reviewer_line_is_drawn_only_where_its_key_works() {
        let mut app = model();
        app.open_localmind(localmind_fixture());
        let backend = TestBackend::new(90, 24);
        let mut terminal = Terminal::new(backend).expect("test terminal");
        ...
        for section in [
            crate::LocalMindSection::Docs,
            crate::LocalMindSection::Graph,
            crate::LocalMindSection::Memory,
        ] {
            assert_eq!(app.localmind_section(), Some(section));
            terminal
                .draw(|frame| {
                    let _ = render(frame, &app);
                })
            ...
            assert!(
                !terminal.backend().to_string().contains("Reviewer:"),
                "{section:?} must not offer the reviewer key"
            );
            let _ = app.handle_input(crate::InputAction::AcceptCompletion, 80);
        }
        ...
        terminal
            ...
            .expect("render review");
        assert!(terminal.backend().to_string().contains("Reviewer: not set"));
    }

    /// While an operation is in flight the host refuses the review actions.
    /// The footer says so rather than advertising three keys that will be
    /// refused without a word.
    #[test]
    fn the_review_footer_reports_a_busy_host_instead_of_offering_refused_keys() {
        let mut app = model();
        app.open_localmind(localmind_fixture());
        for _ in 0..3 {
            let _ = app.handle_input(crate::InputAction::AcceptCompletion, 80);
        ...
        terminal
            ...
            .expect("render busy review");
        let rendered = terminal.backend().to_string();
        assert!(rendered.contains("actions resume when idle"), "{rendered}");
