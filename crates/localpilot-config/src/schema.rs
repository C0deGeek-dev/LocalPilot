...
pub struct ResearchWebConfig {
    ...
    pub enabled: bool,
    /// Domains that may be fetched **without asking** — standing approval, given
    /// in advance by writing them here.
    ///
    /// Empty by default, so every host is asked about per request (ADR-0170).
    /// It used to default to `["*"]`, the open web, which meant a per-request
    /// prompt could never fire on a default install: the gate existed and
    /// nothing ever reached it. Set `["*"]` explicitly to restore silent
    /// open-web reach. `*` matches every host;
...
pub struct MemoryConfig {
    ...
    ///
    /// **A hit that contains the query's subject is exempt.** The floor is a
    /// second opinion on a hit that matched only the shape of the question; a hit
    /// that matched what the question is *about* has already answered it.
    ///
    /// The default is calibrated, not guessed: on a real store, prompts whose
    /// subject appears in no memory retrieve an irrelevant one at cosines of
    /// 0.27–0.35, so a floor just above that band is what separates them. It was
    /// `0.6` while the gate was inert; measured, `0.6` cuts recall on
    /// cross-cutting prompts by two thirds, and there is nothing above the noise
    /// band it buys.
...
mod tests {
    ...
    #[test]
    fn research_web_defaults_on_but_asks_before_every_host() {
        let config = ResearchWebConfig::default();
        assert!(config.enabled, "web research is on by default");
        // Reach is on; *silent* reach is not. An unset allowlist used to mean
        // the open web, which left the per-request prompt unable to fire on a
        // default install — the gate existed and nothing reached it.
        assert!(
            config.allowlist.is_empty(),
            "unset allowlist means every host is asked about"
        );
    ...
    #[test]
    fn an_explicit_open_web_allowlist_is_preserved() {
        // The inverse of the old test. Silent open-web reach is still available,
        // but it now has to be written down — which is the point: it is a
        // standing approval, and a standing approval should be a deliberate act
        // rather than what happens when nobody says anything.
        let parsed: ResearchWebConfig =
            serde_json::from_value(json!({ "allowlist": ["*"] })).unwrap();
