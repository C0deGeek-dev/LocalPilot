...
If the existing context-compaction retry still receives a provider
payload/context-limit rejection and carries multiple images, it takes rung 6
before provider degradation: the next request keeps only the newest image. This
is a request projection only; stored conversation history is unchanged.
Text-only and single-image failures skip this rung.
...
- Human decision reports (`skills available` and the relevant-skill section of
  `skills research`) render each match as an identity line plus its complete,
  indented description, with a blank line between matches. The description is
  deliberately uncapped here; lean locator surfaces (`skills list` and the
  model-facing skill tools) remain summarized. On a terminal, name and state may
  also use the configured semantic palette, but text labels and spacing carry
  the same distinctions in no-color and captured output.
