...
- `cargo test -p localpilot --features tui,learning --bin localpilot` covers the
  ...
  values, contained quick/full help, help wheel/thumb navigation, local refusal
  during active work, and the invariant that slash input never enters the
  provider FIFO or prompt-history store.
