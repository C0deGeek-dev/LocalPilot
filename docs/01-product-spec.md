...
each file-creation approval. This refusal includes the LocalMind Graph panel's
manual `r` reindex action; it never starts a graph worker or creates graph state
in incognito. When the session ends it reports every file it
