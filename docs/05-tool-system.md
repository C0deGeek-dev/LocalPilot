...
accepted memory, recent-session facts, and code structure. They are labelled
**`session transcript`** in results, and the label is load-bearing: a span is
conversation, not a fact the project asserts, and the label is the only thing
separating the two when a model reads a result list.

Spans hold **no budget reserve**. Every other source has a guaranteed share of
the context pack; spans compete only in the shared pool. A reserve is a claim
that a source earns its space, and nothing has yet measured whether spans do.

A span id looks like `span:<session>:<version>:<ordinal>` and is **fetchable**
through `knowledge_fetch`, which resolves it against the transcript it points
into — the span index stores locators and relevance, never text.

A locator can fail to resolve, and the reason is always reported rather than
returned as silence:

| Reason | What happened |
|---|---|
| older chunking contract | Re-chunking renumbered ordinals, so the position no longer identifies the same text |
| session or position gone | The session was deleted, or its transcript no longer reaches that ordinal |
| transcript changed | The transcript was edited after indexing, so that position now holds different text |

In each case the answer is "search again for a current locator", never text
...
The session runtime evaluates the `check_before_launch` rule in its explicit
pre-dispatch decision (see [`docs/06-harness-spec.md`](06-harness-spec.md)). When
the task prompt named a local serveable target (a loopback host, or any
...
## Tool Contract Retry Boundary

`ToolContract` deliberately carries no generic automatic-retry policy. That
field existed briefly as unused vocabulary: no tool declared it and no runtime
read it. It was removed on 2026-08-28 rather than adding a second retry loop at
the registry boundary.

Retries remain owned by the layer that can classify the failure and enforce its
own budget: the model repairs malformed arguments, the harness retries or
replans failed steps, and provider adapters honor provider-specific transient
errors and reset windows. The registry cannot infer that a failed invocation
had no partial effect, so it never repeats a tool call automatically.

`Idempotency` is kept, and it is worth being exact about its status rather than
implying a consumer it does not have. Every builtin declares it; with `retry`
gone, nothing reads it — producers and no consumer, which is the same shape as
the field this section retires. It stays for two reasons: declaring it is how
per-tool knowledge that is expensive to reconstruct stays recorded, and it is
the input a future check would need (a verifier refusing to re-run a
non-idempotent tool, for instance). It is not permission to retry by itself,
and if it acquires no reader it is the next candidate for this same decision
rather than a standing exception.

## Tool Contract Confirmation Model

Confirmation is a tighten-only layer on top of permission policy. Under normal
profiles, an irreversible tool or a tool declaring `Confirmation::Always`
raises an otherwise automatic `Allow` decision to an interactive prompt. A
denial stays a denial, and `Bypass` / `Unrestricted` keep their explicit
no-prompt semantics.

`git_commit` uses `Confirmation::Always`: creating durable repository history
is consequential even though that history makes the operation reversible with
an artifact. Irreversible tools continue to prompt through their reversibility
contract. There is no risk-floor variant; no project risk-floor model exists,
so the former unused `AboveRiskFloor` vocabulary was removed rather than
implying one.

## Tool Contract Preconditions

Contract preconditions run before permission and can only refuse a call. State
predicates are always enforced: `FileExists` resolves the named argument inside
the workspace and requires an existing file. `edit_file` declares it because its
operation is defined only for an existing target.

There is no clean-worktree predicate. The unused `CleanWorktree` vocabulary was
removed on 2026-08-28 rather than left implying a requirement no tool can carry.
The two candidates named when it was written do not survive contact: `git_commit`
commits *staged* changes, and `git_add` and `git_restore` need a dirty tree, so
the predicate would make them permanently unrunnable; `apply_patch` parses and
applies diffs itself without invoking Git, so requiring a worktree would break it
in the plain directories it supports today. The check was also evaluated by
shelling out to `git status`, which binds any declaring tool to a Git workspace —
a heavy, hidden requirement for a predicate that reads as "the tree is clean".
`D004` retains `State` itself, which keeps its producer; `StatePredicate` stays
`#[non_exhaustive]`, so a tool that genuinely needs the check — a branch switch,
say — can reintroduce the variant together with its producer.

`RequiresPriorRead` remains separately controlled by the
`[tools] enforce_prior_read` setting. Disabling that experimental discipline
does not disable state predicates declared by the same tool.

## Tool Contract Verification Methods

The post-execution verifier reads `ToolContract::verification`; the field is not
advisory decoration. `Postconditions` evaluates every declared deterministic
postcondition. `ReadBack { tool: "read_file" }` performs the same bounded,
workspace-contained read semantics and evaluates the contract's read checks
without opening a second permission path. An unsupported read tool,
`ModelCritic` without a configured implementation, and `Unverifiable` all yield
`Unverified`, never success. A tool result that already reports an error yields
`Failed` before method dispatch.

The five built-in file mutators that select read-back verification
(`write_file`, `append_file`, `edit_file`, `multi_edit`, and `replace_in_file`)
also declare `ConfirmRead(NonEmpty)`. A missing, unreadable, or empty target
therefore fails the recorded verdict even when the tool reported success.

The verifier also calls `is_verification_declared` in production. A check-driven
method with no postconditions therefore cannot accidentally license a success
claim. The claim gate remains separately opt-in; verification verdicts are
always recorded so its false-positive rate can be measured.
