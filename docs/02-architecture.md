...
- the parser (`parse_slash_for` and its helpers, plus
  ...
  typed `r` intent uses that same host boundary: the CLI resolves store and
  incognito state, applies the live permission engine, drives bounded blocking
  reindex batches with progress/cancellation, and refreshes neutral view data in
  place (ADR-0174). The full-screen host owns a typed
...
- skill manifests
- quota wait records
- versioned provider-declaration snapshots; their shape is owned by the
  [provider contract](04-provider-contract.md), and the live declaration remains
  authoritative
...
`SessionRuntime` synchronizes the active provider snapshot when a session opens
and after a provider switch. A disk store writes it under `providers/`; an
incognito store keeps the identical record only in its process-local map, so the
privacy boundary is enforced by the backend rather than by a second caller path
(ADR-0160).
...
- project files
- provider metadata (the versioned declaration snapshot described by the
  provider contract, not provider-specific message content)
