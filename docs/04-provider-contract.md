...
At session open, and again when a live session switches provider, LocalPilot
persists a versioned, redacted snapshot of that declaration under
`.localpilot/providers/<id>.json`. The runtime reads the existing snapshot first
and leaves an identical record untouched; a changed live declaration refreshes
it. The live declaration remains authoritative for behavior — this cache is an
inspectable execution record, not a stale-capability fallback. It contains no
credentials and is distinct from provider-specific metadata carried inside a
message or tool call. Incognito sessions exercise the same read/write path
through the process-local `Store::ephemeral` backend and create no provider file.
