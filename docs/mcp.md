...
Servers that advertise the MCP `resources` capability also add two ordinary
session tools. `<server>_resources_list` discovers resource URIs (and accepts a
pagination `cursor`), while `<server>_resource_read` reads a URI returned by the
list tool. For a server configured as `files`, for example, the model can call
`files_resources_list` and then `files_resource_read`. Both calls use the same
permission, redaction, output-size, and retention pipeline as every other tool.
Resource-only servers are supported; they do not need to advertise `tools`.
...
Only local servers launched over stdio are supported. Their tools and resources
are available to the interactive REPL, `print`, and `harness` runs; harness
connects each server once and reuses it across steps.
