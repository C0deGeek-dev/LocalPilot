...
These are internal lifecycle points used by the built-in rule engine, not names
that can be bound to arbitrary user-defined rules. `[harness.rules]` configures
the severity of a named built-in rule; it does not define a rule or select its
trigger. Lifecycle points without a runtime emitter are not part of the
advertised surface.
...
> choke-point — `ToolRegistry::dispatch → PermissionEngine::decide` — which every file
...
>
> **There is no generic in-process tool-gate extension point.** The former seam
> had no production registrant and could not host the post-execution verifier or
...
An adapter's length/max-token stop is provisional until the request's usage is
known. When the reported prompt plus output fills the declared context window
while output remains below half the configured output cap, the binding limit is
the context window, not the cap. The harness discards the partial reply,
compacts active history, and retries once through the overflow-recovery path. It
does not append the chunked-write prompt, and it never advises raising
`max_tokens` for this condition (that would reduce the history budget). If no
completed history can be removed, or the compacted retry fills the window again,
the turn stops with the actual prompt/output/window counts and tells the user to
start a new session or split the task. Pinned by the context-exhaustion session
tests, including an output-limit event that arrives before its usage report.
